...
function loadOneWorldItem(id1)
	local row = exports.cr_mysql:query_fetch_assoc("SELECT `id`, `itemid`, `itemvalue`, `x`, `y`, `z`, `dimension`, `interior`, `rx`, `ry`, `rz`, `creator`, `creationdate`, `protected`, `perm_use`, `perm_move`, `perm_pickup`, `perm_use_data`, `perm_move_data`, `perm_pickup_data` FROM `worlditems` WHERE `id`='" .. id1 .. "' ")
	...
	if row then
		...
		if itemID < 0 then -- weapon
			...
			local modelid = exports['cr_items']:getItemModel(itemID, itemValue)
			...
			local rx, ry, rz, zoffset = exports['cr_items']:getItemRotInfo(itemID)
			local obj = createItem(id, itemID, itemValue, modelid, x, y, z + (zoffset or 0), rx+rx2, ry+ry2, rz+rz2)
			
			if isElement(obj) then
				exports.cr_pool:allocateElement(obj, itemID, true)
				setElementDimension(obj, dimension)
				setElementInterior(obj, interior)
				setElementData(obj, "creator", creator)
				setElementData(obj, "createdDate", createdDate)
...
function loadWorldItems(res)
	...
	--exports.cr_mysql:query_free("DELETE FROM `worlditems` WHERE DATEDIFF(NOW(), creationdate) > 30 AND `itemID` != 80 AND `itemID` != 81 AND `itemID` != 103 AND protected = 0")
	
	exports.cr_mysql:query_free("DELETE FROM `worlditems` WHERE `protected`='0' AND `itemID` NOT IN(81, 103, 169) AND ((DATEDIFF(NOW(), creationdate) > 30) OR (DATEDIFF(NOW(), creationdate) > 7 AND `itemID` = 72)) ")
	
	-- actually load items
	local result = exports.cr_mysql:query("SELECT `id` FROM `worlditems`")
	...
	setTimer(restartResource, timerDelay+60000, 1, getResourceFromName("cr_item-texture")) -- Restart item texture resource 60 seconds after all world items loading done. / Farid
end
