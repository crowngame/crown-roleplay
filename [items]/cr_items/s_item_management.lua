...
function getMaxWeight(element)
	...
	elseif getElementType(element) == "vehicle" then
		...
		elseif isTruck(element) then
			return tonumber(exports["cr_job-trucker"]:getTruckCapacity(element)) or 120
		else
			return 20
		end
...
function fixInventory(thePlayer, commandName)
	if not isTimer(fixInventoryTimer[thePlayer]) then
		if (not getElementData(thePlayer, "baygin")) and (not getElementData(thePlayer, "adminjailed")) then
