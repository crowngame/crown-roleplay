...
local fonts = {
	exports.cr_fonts:getFont("sf-regular", 9),
	exports.cr_fonts:getFont("sf-regular", 11)
}

local font = exports.cr_fonts:getFont("sf-regular", 10)
...
function renameWeapon(item)
	...
	if (checkString == " (D)")  then
		...
		guiEditSetMaxLength(nameEdit, 25)
			if weaponName ~= name and not exports.cr_integration:isPlayerGameAdmin(localPlayer) then 
		...
		elseif calibre and string.len(calibre) > 3 and not exports.cr_integration:isPlayerGameAdmin(localPlayer) then
			guiSetEnabled(calibreEdit, false)
		end
		if calibre then
