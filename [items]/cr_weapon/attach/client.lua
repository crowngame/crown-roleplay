...
function createSelectionWindow()
	...
	sWin = guiCreateWindow(sx - 225, sy - 250, 215, 202, "Crown Roleplay - Silah Konum", false)
		...
		addEventHandler("onClientGUIChanged", posEditBox[i],
			function ()
				...
				if math.abs(x) > 0.5 or math.abs(y) > 0.5 or math.abs(z) > 0.5 then
					...
					if isElement(editingObject) then
						exports.cr_boneattach:attachElementToBone(editingObject, localPlayer, bone, x, y, z, rx, ry, rz)
					end
...
function createEditingObject(weapon, bone, x, y, z, rx, ry, rz)
	...
	exports.cr_boneattach:attachElementToBone(object, localPlayer, bone, x, y, z, rx, ry, rz)
...
function saveWeaponAttachment(button, state)
	if button == "left" and state == "up" then
		...
		if isElement(editingObject) then
			exports.cr_boneattach:attachElementToBone(editingObject,
