...
addEventHandler("frames:delete", resourceRoot,
	function(id)
		...
		if global:hasItem(client, 4, interior) or global:hasItem(client, 5, interior) or (integration:isPlayerGameAdmin(client) and global:isAdminOnDuty(client)) or (interior==0) then
...
function newTexture(source, url, texture)
	...
	if dimension >= 1 or (integration:isPlayerHeadAdmin(source) and global:isAdminOnDuty(source)) or integration:isPlayerScripter(source) then
		if global:hasItem(source, 4, dimension) or global:hasItem(source, 5, dimension) or (integration:isPlayerGameAdmin(source) and global:isAdminOnDuty(source)) or (dimension==0) then
