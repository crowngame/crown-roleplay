...
function frames_showTextureSelection (slot, textureURL, imgData)
	...
	if dimension >= 1 or (integration:isPlayerHeadAdmin(localPlayer) and global:isAdminOnDuty(localPlayer)) or integration:isPlayerScripter(localPlayer) then
		if exports.cr_global:hasItem (localPlayer, 4, dimension) or exports.cr_global:hasItem (localPlayer, 5, dimension) or (integration:isPlayerGameAdmin(localPlayer) and global:isAdminOnDuty(localPlayer)) or (dimension==0) then
