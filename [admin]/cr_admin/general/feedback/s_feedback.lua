...
function openFeedBackDetails(staff)
	if staff then
		...
		if target and target.id and tonumber(target.id) then
			...
			if target.id ~= getElementData(source, "account:id") and not exports.cr_integration:isPlayerHeadAdmin(source) then
...
function showFeedbacks(player, cmd, ...)
	if (...) then
		if not exports.cr_integration:isPlayerHeadAdmin(player) then
