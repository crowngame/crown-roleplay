...
local impounderCol = {
	...
	[59] = createColCuboid (795.4951171875, -621.00390625, 10.3359375, 4.4, 7, 10),
}
...
releasePositions = {
	...
	[4] = { 
        { 2687.5, -2531.3999, 13.5, 0, 0, 90 },
        { 2687.5, -2527.3, 13.5, 0, 0, 90 },
        { 2687.6001, -2522.7, 13.4, 0, 0, 90 },
        { 2687.8, -2518.6001, 13.3, 0, 0, 90 },
        { 2687.8999, -2514.3, 13.2, 0, 0, 90 },
        { 2670.3999, -2516.5, 13.3, 0, 0, 270 },
        { 2670.6001, -2530, 13.5, 0, 0, 270 },
        { 2670.8, -2521.7, 13.6, 0, 0, 270 },
        { 2670.7, -2525.8999, 13.6, 0, 0, 270 },
    }
}
...
function pedSay(pedName, convoId)
	return exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. (getConvoText(convoId)), 255, 255, 255, 10)
end
...
function openImpGui(pedName)
	...
	if not exports.cr_global:hasItem(source, badgeId) then
		return exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. getConvoText('nobadge'), 255, 255, 255, 10)
	end
	local found = getVehicleWithinBooth(factionId)
	if not found then
		return exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. getConvoText('noveh'), 255, 255, 255, 10)
	end
	local freeLane, used, total = getAFreeLane(factionId)
	if not freeLane then
		return exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. getConvoText('The impound lot here is full, maybe release some or impound this else where?'), 255, 255, 255, 10)
	end
	local vehName = exports.cr_global:getVehicleName(found)
	local vehId = getElementData(found, "dbid")
	--exports.cr_global:sendLocalText(source, "" .. pedName .. ": Alright, let's see. Hm.. " .. vehName .. ", right?", 255, 255, 255, 10)
...
function leoStartImpounding(dep, vehid, laneNumber, days, fine, first, last, badge, rank, vehName, plate, vin, volations, location, other)
	...
	if not freeLane or freeLane.veh ~= "0" then
		exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. getConvoText('lane_not_free_anymore'), 255, 255, 255, 10)
		triggerClientEvent(source, "tow:reEnableImpGui", source)
		return false
	end

	local veh = exports.cr_pool:getElement("vehicle", vehid)
	if getVehicleWithinBooth(factionId) ~= veh then
		triggerClientEvent(source, "tow:reEnableImpGui", source, true)
		exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. getConvoText('one_car_at_once'), 255, 255, 255, 10)
		return false
	end
	--unlock it
	local time = getRealTime()
    setElementData(veh, "Impounded", time.yearday)
    ...
    exports.cr_global:giveItem(veh, 72, "Towing Notice: Impounded by " ..  rank  .. " '" ..  first  .. " " .. last .. "', Violation(s): '" .. volations .. "' at '" .. datestr .. "', Releasing in next " .. days .. " day(s).")
    --Impound it
	...
	setElementData(veh, "respawnposition", {freeLane.x, freeLane.y, freeLane.z, freeLane.rx, freeLane.ry, freeLane.rz}, false)
	setElementData(veh, "interior", freeLane.int)
	setElementData(veh, "dimension", freeLane.dim)
	...
	--Respawn it
	setElementData(veh, 'i:left')
	setElementData(veh, 'i:right')
	respawnVehicle(veh)
	...
	setVehicleLocked(veh, false)
	setElementData(veh, "handbrake", 1, true)
	...
	setElementData(veh, "impounder", factionId, false, true)
	...
	report = report .. "- Fine: $" .. exports.cr_global:formatMoney(tonumber(fine)) .. "\n"
	...
	--notifications
	exports.cr_global:sendLocalDoAction(source, "Vehicle " .. vehName .. " has been impounded to " .. factionName .. "'s impound lot at lane #" .. freeLane.lane .. ".")
	local owner = getElementData(veh, "owner")
	local account = exports.cr_cache:getAccountFromCharacterId(owner) or {id = 0, username="No-one"}
	local characterName = exports.cr_cache:getCharacterNameFromID(owner) or "No-one"
	if owner > 0 and account then
		exports.cr_announcement:makePlayerNotification(account.id, factionName .. " has " .. (tonumber(days)==0 and "seized" or "impounded") .. " your " .. vehName .. " (owned by " .. characterName .. ")" .. (tonumber(days)>0 and (" for " .. days .. " day(s).") or "."))
	end

	--Leave some logs
	exports.cr_logs:dbLog(source, 6, {  veh }, "LEO IMPOUNDED " .. days .. " DAYS")
	exports['cr_vehicle-manager']:addVehicleLogs(vehid, "LEO IMPOUNDED " .. days .. " DAYS", source)
	--Close GUI
	triggerClientEvent(source, "tow:reEnableImpGui", source, true)
end
...
function getAFreeLane(factionId)
	...
	local q = exports.cr_mysql:query("SELECT * FROM leo_impound_lot " .. (factionId and ("WHERE faction=" .. factionId) or "") .. " ORDER BY lane")
	while q do
		local lane = exports.cr_mysql:fetch_assoc(q)
	...
	exports.cr_mysql:free_result(q)
	return foundLane, countLaneUsed, countLane
end

function unimpVeh(vehid)
	local veh = exports.cr_pool:getElement("vehicle", tonumber(vehid))
	if veh then
		...
		if impounder == 1 or impounder == 59 then
			exports.cr_mysql:query_free("UPDATE vehicles SET Impounded=0 WHERE id=" .. vehid)
            ...
            setElementData(veh, "Impounded", 0, true)
			setElementData(veh, "impounder", nil, false, true)

            updateVehPos(veh)
			setElementData(veh, "interior", int)
			setElementData(veh, "dimension", dim)
			...
			if source then
				--Create report
				local report = ""
				local thefaction = exports.cr_pool:getElement("team", getElementData(source, "faction"))
				local factionName = getTeamName(thefaction) or "Unknown Organization"
				report = report..factionName .. " - Trooper's Impound Report\n"
				report = report .. "\nReleased by " .. exports.cr_global:getPlayerName(source)
				exports.cr_mysql:query_free("INSERT INTO mdc_impounds SET veh=" .. vehid .. ", content='" .. exports.cr_mysql:escape_string(report) .. "', reporter='" .. exports.cr_mysql:escape_string(exports.cr_global:getPlayerName(source)) .. "' ")
			end
            exports.cr_mysql:query_free("UPDATE leo_impound_lot SET veh=0, release_date=NULL, fine=0 WHERE veh=" .. vehid)
	...
	for factionId, laness in pairs(lanes) do
		for i, lane in pairs(laness) do
			if tonumber(lane.veh) ~= 0 then
				local veh = exports.cr_pool:getElement("vehicle", tonumber(lane.veh))
				if not veh or not isElement(veh) or getElementData(veh, "Impounded") == 0 or getElementData(veh, "impounder") ~= factionId then
					...
					exports.cr_mysql:query_free("UPDATE leo_impound_lot SET veh=0, release_date=NULL, fine=0 WHERE lane=" .. lane.lane)
...
function addlane(player, cmd, fid)
		...
		if exports.cr_mysql:query_free("INSERT INTO leo_impound_lot SET x=" .. x .. ", y=" .. y .. ", z=" .. z .. ", rx=" .. rx .. ", ry=" .. ry .. ", rz=" .. rz .. ", `int`=" .. int .. ", dim=" .. dim .. ", faction=" .. fid) then
...
function getLaneFromVeh(vehid)
	return exports.cr_mysql:query_fetch_assoc("SELECT * FROM leo_impound_lot WHERE veh=" .. vehid)
end
...
function openReleaseGUI(pedName)
	...
	local vehs = {}
	local empty = true
	for i, veh in pairs(exports.cr_pool:getPoolElementsByType("vehicle")) do
		local oneVeh = {}
		local owner = getElementData(veh, "owner") 
		...
		if owner == getElementData(source, "dbid") and impounded ~= 0 and impounder == factionId then
			...
			oneVeh.model = exports.cr_global:getVehicleName(veh)
	...
	if not empty then
		...
		return exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. (getConvoText("no_car_to_release")), 255, 255, 255, 10)
	end
end
...
function release(pedName, vehid, cost)
	...
	local lane = exports.cr_mysql:query_fetch_assoc("SELECT CASE WHEN release_date IS NULL THEN 'seized' ELSE TO_SECONDS(release_date) END AS secdiff FROM leo_impound_lot WHERE veh=" .. vehid .. " LIMIT 1")
	...
	if lane.secdiff == "seized" then
		exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. (getConvoText("I'm sorry your vehicle is seized, I can not release it until further notice, you gotta contact our officiers about that.")), 255, 255, 255, 10)
		return false
	end
	local text, sec = exports.cr_datetime:formatFutureTimeInterval(tonumber(lane.secdiff))
	if sec ~= 0 then
		exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. (getConvoText("Well, according to the document I got here, you gotta wait " ..  text .. " more to release this vehicle. Sorry .. ")), 255, 255, 255, 10)
		return false
	end

	if not exports.cr_global:takeMoney(source, cost) then
		exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. (getConvoText("$" .. exports.cr_global:formatMoney(cost) .. " please .. ")), 255, 255, 255, 10)
		return false
	end

	local veh = exports.cr_pool:getElement("vehicle", vehid)
	local impounder = getElementData(veh, "impounder")
	local thefaction = exports.cr_pool:getElement("team", impounder)
	if thefaction then
		if exports.cr_bank:giveBankMoney(thefaction, cost) then
			exports.cr_bank:addBankTransactionLog(getElementData(source,"dbid"), -getElementData(thefaction, "id"), cost, 2, "Impound fee & fine to release vehicle VIN #" .. vehid)
			exports.cr_logs:dbLog(source, 25 , {thefaction}, "$" .. exports.cr_global:formatMoney(cost) .. " Impound fee & fine to release vehicle VIN #" .. vehid)
		end
	end

	local state, reason = unimpVeh(vehid)
	if state then
		exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. (getConvoText("Your vehicle has been released, it's out front. ((Please remember to /park your vehicle so it does not respawn back here.))")), 255, 255, 255, 10)
		exports['cr_vehicle-manager']:addVehicleLogs(vehid, "LEO UNIMPOUND.", source)
		exports.cr_logs:dbLog(source, 6, {  veh }, "LEO UNIMPOUNDED ")
	else
		exports.cr_global:sendLocalText(source, "" .. pedName .. ": " .. (getConvoText("Opps, sorry. " .. reason)), 255, 255, 255, 10)
	end
end
