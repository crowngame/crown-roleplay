...
local pdReleasePed = createPed(280, 238.9697265625, 112.8681640625, 1003.21875)
setPedRotation(pdReleasePed, 270)
setElementDimension(pdReleasePed, 1158)
setElementInterior(pdReleasePed , 10)
...
function openImpGui(dep, vehid, first, last, vehName, plate, vin, laneData)
	...
	gui.window[1] = guiCreateWindow(642, 140, 804, 405, dep .. " Impound Lot ((Vehicle ID " .. vehid .. ")) [v1.0 by Farid]", false)
	guiWindowSetSizable(gui.window[1], false)
	exports.cr_global:centerWindow(gui.window[1])
...
function openReleaseGUI(factionId, vehs)
	...
	exports.cr_global:centerWindow(rgui.window[1])
	...
	addEventHandler("onClientGUIClick", rgui.window[1], function()
		...
		if source == rgui.button[1] then
			if selectedRow == -1 or selectedCol == -1 then
				...
				if exports.cr_global:hasMoney(localPlayer, cost) then
					triggerServerEvent("tow:release", localPlayer, pedName, selectedvin, cost)
					closeRgui()
					exports.cr_global:playSoundSuccess()
				else
					triggerServerEvent("tow:pedSay", localPlayer, pedName, "May I have $" .. exports.cr_global:formatMoney(cost) .. " please?")
				end
			end
		...
		elseif source == rgui.gridlist[1] then
			if selectedvin then
				...
				guiSetText(rgui.label[2], "Total: $" .. exports.cr_global:formatMoney(cost))
