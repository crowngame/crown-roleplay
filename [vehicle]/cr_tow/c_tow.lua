...
--local ped = createPed(240, 1804.3212890625, -1381.7099609375, 29.236000061035)
--setElementInterior(ped, 3)
--setElementDimension(ped, 14)
--setElementRotation(ped, 0, 0, 146.80981445313, "default", true)
...
function showImpoundUI(vehElementsret)
	if not wImpound then
		...
		for key, value in ipairs(vehElements) do
			...
			guiGridListSetItemText(gCars, row, col, exports.cr_global:getVehicleName(value), false, false)
...
function updateCar()
	...
	if (row~=-1) and (col~=-1) then
		...
		if not exports.cr_global:hasMoney(getLocalPlayer(), 250) and exports.cr_global:hasItem(getLocalPlayer(), 3, guiGridListGetItemText(gCars, row, IDcolumn)) then
