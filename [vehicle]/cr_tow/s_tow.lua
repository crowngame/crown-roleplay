...
local function insertTowRecord(player, vehicle)
    local plate = 'NULL'
    if exports['cr_vehicle']:hasVehiclePlates(vehicle) and getElementData(vehicle, "show_plate") ~= 0 then
                    ...
                    if (source == towSphere2) then
                        ...
                        if (getElementData(temp, "faction") ~= 4) then
                            if (impounded == 0) then
                                ...
                                local theTeam = exports.cr_pool:getElement("team", 4)
                                local factionRanks = getElementData(theTeam, "ranks")
                                local factionRank = factionRanks[getElementData(thePlayer, "factionrank")] or ""
                                
                                exports.cr_global:giveItem(temp, 72, "Towing Notice: Impounded by " ..  factionRank  .. " '" ..  getPlayerName(thePlayer)  .. "' at " .. datestr)
                                ...
                                if owner > 0 and account then
                                    exports.cr_announcement:makePlayerNotification(account.id, factionName .. "has impounded your " .. exports.cr_global:getVehicleName(temp) .. " (owned by " .. characterName .. ").")
                                end
                            end
                        end
                    end
                            ...
                            if rank >= 5 then
                                if (getElementData(veh, "faction") ~= 4) then
                                    if (impounded == 0) then
                                        ...
                                        exports.cr_logs:logMessage("[IMPOUNDED BIKE] " .. getPlayerName(player) .. " impounded vehicle #" .. dbid .. ", owned by " .. tostring(exports['cr_cache']:getCharacterName(owner)) .. ", in " .. table.concat({exports.cr_global:getElementZoneName(veh)}, ", ") .. " (pos = " .. table.concat({getElementPosition(veh)}, ", ") .. ", rot = " ..  table.concat({getVehicleRotation(veh)}, ", ") .. ", health = " .. getElementHealth(veh) .. ")", 14)
...
function payRelease(vehID)
    if exports.cr_global:takeMoney(source, 1000) then
        exports.cr_global:giveMoney(getTeamFromName("Rapid Towing"), 1000)
        exports.cr_mysql:query_free("INSERT INTO wiretransfers (`from`, `to`, `amount`, `reason`, `type`) VALUES (" .. exports.cr_mysql:escape_string(getElementData(source, "dbid")) .. ", " .. -getElementData(getTeamFromName("Rapid Towing"), "id") .. ", " .. 1000 .. ", 'Vehicle Release', 5)")
        ...
        setElementData(vehID, "enginebroke", 0, true)
        setVehicleDamageProof(vehID, false)
        setVehicleEngineState(vehID, false)
        setElementData(vehID, "handbrake", 0, true)
...
function unimpoundVeh(thePlayer, commandName, vehid)
    if thePlayer then
        if not vehid then
            ...
            local vehID = exports.cr_pool:getElement("vehicle", tonumber(vehid))
            if not vehID then
                    ...
                    if commandName == "unimpound" then
                        if (getElementData(thePlayer,"faction") == 4) then
                            if(getElementData(thePlayer, "factionleader") == 1) then
                                ...
                                exports.cr_logs:logMessage("[SFTR-UNIMPOUND] " .. username .. " has unimpounded vehicle #" .. vehid .. ".", 9)
                            else
                    ...
                    elseif commandName == "aunimpound" then
                        if exports.cr_integration:isPlayerTrialAdmin(thePlayer)  then
                            ...
                            if impounder == 1 or impounder == 59 then
                                ...
                                outputChatBox("You have unimpounded vehicle #" .. vehid .. ".", thePlayer, 0, 255, 0)
                                exports.cr_global:sendMessageToAdmins("[ADM] " .. adminTitle .. " " .. playerName .. " unimpounded vehicle #" .. vehid .. ".")
                                
                                exports.cr_logs:logMessage("[ADMIN-UNIMPOUND] " .. playerName .. " has unimpounded vehicle #" .. vehid .. ".", 9)
                            end
                        end
                    end
                else
...
function releaseHandbrake(theTruck) -- Release handbrake on the vehicle being attached or being towed / Farid
    if getElementData(source, "handbrake") ~= 0 then
        setElementData(source, "handbrake", 0, true)
...
function triggerShowImpound()
    ...
    local vehElements = {}
    local count = 1
    for key, value in pairs(exports.cr_pool:getPoolElementsByType("vehicle")) do
...
function updateTowingVehicle(theTruck)
    ...
    if (thePlayer) then
        if (getElementData(thePlayer,"faction") == 4) or (getElementData(thePlayer,"faction") == 42) then
            ...
            local carName = exports.cr_global:getVehicleName(source)
        ...
        if thePlayer then
            exports.cr_logs:logMessage("[TOW] " .. getPlayerName(thePlayer) .. " started towing vehicle #" .. getElementData(source, "dbid") .. ", owned by " .. tostring(exports['cr_cache']:getCharacterName(getElementData(source,"owner"))) .. ", from " .. table.concat({exports.cr_global:getElementZoneName(source)}, ", ") .. " (pos = " .. table.concat({getElementPosition(source)}, ", ") .. ", rot = " ..  table.concat({getVehicleRotation(source)}, ", ") .. ", health = " .. getElementHealth(source) .. ")", 14)
        end
    end
end
...
function updateCivilianVehicles(theTruck)
    if (isElementWithinColShape(theTruck, towSphere)) then
        ...
        if (dbid >= 0 and faction == -1 and owner < 0) then
            exports.cr_global:giveMoney(exports.cr_pool:getElement("team", 4), 200)
    ...
    if getVehicleOccupant(theTruck) then
        exports.cr_logs:logMessage("[TOW STOP] " .. getPlayerName(getVehicleOccupant(theTruck)) .. " stopped towing vehicle #" .. getElementData(source, "dbid") .. ", owned by " .. tostring(exports['cr_cache']:getCharacterName(getElementData(source,"owner"))) .. ", in " .. table.concat({exports.cr_global:getElementZoneName(source)}, ", ") .. " (pos = " .. table.concat({getElementPosition(source)}, ", ") .. ", rot = " ..  table.concat({getVehicleRotation(source)}, ", ") .. ", health = " .. getElementHealth(source) .. ")", 14)
    end
end
