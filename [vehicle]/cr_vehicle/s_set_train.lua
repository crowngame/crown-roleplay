...
addCommandHandler("settrainrailed",
        function(player, cmd, vehicleID, state)
                if exports.cr_integration:isPlayerDenemeYetkili(player) then
                        local vehicleID = tonumber(vehicleID)
                        local state = tonumber(state)
                        if vehicleID then
                                local vehicle = exports.cr_pool:getElement("vehicle", vehicleID)
                                if vehicle then
                                        if state == 0 then
                                        ...
                                        exports.cr_logs:logMessage("[TRAINRAIL] " .. getPlayerName(player) .. " set a train " .. (state == 0 and "railed" or "derailed") .. " (#" .. vehicleID .. ")", 9)
                                else
                                        outputChatBox("Vehicle not found.", player, 255, 0, 0, false)
                                end
                        else
                                outputChatBox("KULLANIM: /" .. cmd .. " [vehicle id] [0- Railed, 1- Derailed]", player, 255, 194, 14, false)
                        end
                end
        end, false, false
)
                ...
                if exports.cr_integration:isPlayerDenemeYetkili(player) then
                        ...
                        if vehicleID then
                                ...
                                if vehicle then
                                        ...
                                        exports.cr_logs:logMessage("[TRAINDIR] " .. getPlayerName(player) .. " set a train direction to " .. (state == 0 and "clockwise" or "counterclockwise") .. " (#" .. vehicleID .. ")", 9)
                                else
                                        outputChatBox("Vehicle not found.", player, 255, 0, 0, false)
                                end
                        else
                                outputChatBox("KULLANIM: /" .. cmd .. " [vehicle id] [0- CW, 1- CCW]", player, 255, 194, 14, false)
                        end
                end
        end, false, false
)