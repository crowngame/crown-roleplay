...
addCommandHandler("neon", function()
	if isTimer(neonCommandTimer) then return end
...
addCommandHandler("getneon", function()
	outputChatBox(tostring(getElementData(getPedOccupiedVehicle(localPlayer), "tuning.neon")))
	outputChatBox(tostring(getElementData(getPedOccupiedVehicle(localPlayer), "vehicle.neon.active")))
end)
...
addEventHandler("neon.showList", root, function()
	...
	neonGUI = guiCreateWindow(0, 0, 400, 500, "Araç Neon Sistemi", false)
	guiWindowSetSizable(neonGUI, false)
	exports.cr_global:centerWindow(neonGUI)
	...
	ok = guiCreateButton(9, 455, 190, 32, "Satın Al (100 TL)", false, neonGUI)
	...
	addEventHandler("onClientGUIClick", root, function(b)
		if (b == "left") then
			...
			elseif (source == ok) then
				...
				showCursor(false)
				destroyElement(neonGUI)
				triggerServerEvent("market.buyVehicleNeon", localPlayer, vehicleID, neonIndex, 100)
			end
		end
	end)
end)
