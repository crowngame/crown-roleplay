-- // Boratav tarafından yeniden kodlandı.
...
function forceSyncStationsToAllclients()
	...
	if stations and tonumber(stations) and tonumber(stations) > 0 then
		...
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", stations .. " radio station(s) have been successfully synced to " .. syncedClients .. " online clients (" .. failedClients .. " failed).")
	else
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Could not synced any radio stations.")
	end
end
