...
function fetchStations()
	...
	mysql:getConnection(), "SELECT `id`, `station_name`, `source`, (SELECT `username` FROM `` WHERE ``.`id`=`owner`) AS `owner`, `register_date`, `expire_date`, `enabled`, `order` FROM `radio_stations` WHERE (`expire_date` IS NULL) OR (`expire_date` > NOW()) ORDER BY `order` ")
...
function createNewStation(name, ip, donorStation)
	if name and ip then
		if donorStation then
			...
			dbExec(mysql:getConnection(),"INSERT INTO `radio_stations` SET `id`='" .. smallestID .. "', `station_name`='" .. exports.cr_global:toSQL(name) .. "', `source`='" .. exports.cr_global:toSQL(ip) .. "', `order`='" .. smallestID .. "', `owner`='" .. getElementData(client, "account:id") .. "', `expire_date`=(NOW() + interval 30 day) ")
			--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "New radio station has been successfully created! (Name: " .. name .. ", URL: " .. ip .. ")")
			setElementData(client, "gui:ViewingRadioManager", true, true)
			forceUpdateClientsGUI()
		else
			local smallestID = SmallestID()
			dbExec(mysql:getConnection(),"INSERT INTO `radio_stations` SET `id`='" .. smallestID .. "', `station_name`='" .. exports.cr_global:toSQL(name) .. "', `source`='" .. exports.cr_global:toSQL(ip) .. "', `order`='" .. smallestID .. "' ")
			--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "New radio station has been successfully created! (Name: " .. name .. ", URL: " .. ip .. ")")
			forceUpdateClientsGUI()
		end
	else
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Could not create new radio station.")
	end
end
...
function editStation(id, name, ip)
	if id and name and ip and dbExec(mysql:getConnection(),"UPDATE `radio_stations` SET `station_name`='" .. exports.cr_global:toSQL(name) .. "', `source`='" .. exports.cr_global:toSQL(ip) .. "' WHERE `id`='" .. id .. "' ") then
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Updated radio station #" .. id .. " successfully!")
		forceUpdateClientsGUI()
	else
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Could not update radio station #" .. id)
	end
end
...
function deleteStation(id)
	if id and dbExec(mysql:getConnection(),"DELETE FROM `radio_stations` WHERE `id`='" .. id .. "' ") then
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Deleted radio station #" .. id .. ".")
		forceUpdateClientsGUI()
	else
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Could not delete radio station #" .. id .. ".")
	end
end
...
function togStation(id, state)
	if id and state and dbExec(mysql:getConnection(),"UPDATE `radio_stations` SET `enabled`='" .. (state == "Activated" and "1" or "0") .. "' WHERE `id`='" .. id .. "' ") then
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Radio station #" .. id .. " has been " .. state .. "!")
		forceUpdateClientsGUI()
	else
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Could not update radio station #" .. id)
	end
...
function moveStationPosition(id, name, order, movingUp, donorStation)
	if id and tonumber(id) and order and tonumber(order)  then
		...
		if movingUp then
			if order < 2 then
				--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "This radio station is already on top.")
				return false
			end
			if dbExec(mysql:getConnection(),"UPDATE `radio_stations` SET `order`=`order`-1 WHERE `id`='" .. (id) .. "' ") then
				forceUpdateClientsGUI()
				--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Radio station '" .. name .. "' has been moved up!")
			else
				--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Could not move radio station.")
			end
		else
			if dbExec(mysql:getConnection(),"UPDATE `radio_stations` SET `order`=`order`+1 WHERE `id`='" .. (id) .. "' ") then
				forceUpdateClientsGUI()
				--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Radio station '" .. name .. "' has been moved down!")
			else
				--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Could not move radio station.")
			end
		end
	else
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Could not move radio station's position.")
	end
end
...
function renewStation(station, duration)
	if station and duration and tonumber(duration) then
		...
		elseif duration == 90 then
			...
			--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Could not renew radio station #" .. id)
			return false
		end
		...
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "You have successfully renewed Radio Station ID#" .. id .. "!")
		forceUpdateClientsGUI()
	else
		--exports.cr_hud:sendBottomNotification(client, "Radio Station Manager", "Could not renew radio station #" .. id)
	end
