		...
		function(button)
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					if exports.cr_integration:isPlayerHeadAdmin(localPlayer) then
		...
		function(button)
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					...
					if owner == username or exports.cr_integration:isPlayerHeadAdmin(localPlayer) then
		...
		function(button)
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					local username = getElementData(localPlayer, "account:username")
					local owner = guiGridListGetItemText(grid.donorStations , row, 5)
					if owner == username then --or exports.cr_integration:isPlayerHeadAdmin(localPlayer) then
						local id = guiGridListGetItemText(grid.donorStations , row, 1)
						local name = guiGridListGetItemText(grid.donorStations , row, 2)
						local ip = guiGridListGetItemText(grid.donorStations , row, 3)
						local owner = guiGridListGetItemText(grid.donorStations , row, 5)
