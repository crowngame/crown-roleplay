...
function createLicenseQuestionWindow(number)
	...
	if(number < NoQuestionToAnswer) then
		...
		exports["cr_dxlib"]:convertUI(guiQuestionWindow)
...
function createTestFinishWindow()
	...
	exports["cr_dxlib"]:convertUI(guiFinishWindow)
end
...
testRoute = {
	...
	{ 1160.7841796875, -1714.4091796875, 13.433871269226 },	-- Ma808ic St
	{ 1172.8017578125, -1724.697265625, 13.261546134949 },	-- Ma808ic St, turn to Park ave
...
function initiateDrivingTest()
	...
	outputChatBox("#ff2400[!]#FFFFFF Artık araç sürüş testine girmeye hazırsınız, dışarıdaki sürücü araçlarından birisini alın.", 255, 255, 0, true)
	else
				outputChatBox("#ff2400[!]#FFFFFF Zaten bir ehliyetin var.", 255, 0, 0, true)
	end
end
...
function startDrivingTest(element)
	if element == getLocalPlayer() then
		...
		if not (testVehicle[id]) then
			outputChatBox("#ff2400[!]#FFFFFF Sadece ehliyet araçları ile bu bölgeden geçebilirsiniz.", 255, 0, 0, true) -- Wrong car type.
		else
			destroyElement(blip)
			...
			outputChatBox("#ff2400[!]#FFFFFF Test aracına zarar vermeden sürüşü tamamlamanız gerekiyor, bol şans!", 255, 194, 14, true)
		end
	end
end
	...
	if (element == localPlayer) then
		...
		if not (testVehicle[id]) then
			...
			if (tonumber(max_number-1) == tonumber(m_number)) then -- if the next checkpoint is the final checkpoint.
				outputChatBox("#ff2400[!]#FFFFFF Testi tamamlamak için son varış noktasına park edin.", 255, 15, 255, true)
	...
	if (element == localPlayer) then
		...
		if not (testVehicle[id]) then
			...
			if (vehicleHealth >= 800) then
				...
				-- PASS --
				----------
				outputChatBox("#ff2400[!]#FFFFFF Aracı inceledik ve bir hasar göremedik.", 255, 194, 14, true)
				triggerServerEvent("acceptCarLicense", getLocalPlayer())
			else
				----------
				-- Fail --
				----------
				outputChatBox("#ff2400[!]#FFFFFF Araçta ufak tefek çizikler var, iyi bir şoför olman için iyi bir sürücü olman gerekiyor. Yeniden başvurabilirsin.", 255, 0, 0, true)
			end
