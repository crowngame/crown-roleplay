local startCamX, startCamY, startCamZ, startCamLX, startCamLY, startCamLZ = 456.15551757812, -1989.2800292969, 26.29380607605, 382.49728393555, -2055.2377929688, 11.325818061829, 0, 70
local endCamX, endCamY, endCamZ, endCamLX, endCamLY, endCamLZ = 400.54150390625, -2077.3400878906, 10.722999572754, 400.54440307617, -2076.3415527344, 10.667260169983, 0, 70
...
function loadCharacterSelector(characters)
    ...
    if #pedData ~= 0 then
        localCharacters[selectedCharacter] = createPed(pedData[selectedCharacter][5], 400.736328125, -2071.98046875, 10.740701675415, 175.74273681641)
...
addEvent("loadCharacterSelector", true)
addEventHandler("loadCharacterSelector", root, loadCharacterSelector)
...
function characterSelectRender()
    ...
    if access and localCharacters[selectedCharacter] then
		...
		dxDrawRectangle(20, 20, 200, 40, exports.cr_ui:isInBox(20, 20, 200, 40) and tocolor(0, 157, 110, 250) or tocolor(0, 157, 110, 200))
		dxDrawText("Karaktere Gir [ENTER]", 45, 31, nil, nil, tocolor(255, 255, 255, 250), 1, fonts.font3)
		
		if exports.cr_ui:isInBox(20, 20, 200, 40) and getKeyState("mouse1") and click+500 <= getTickCount() then
			click = getTickCount()
		...
		dxDrawRectangle(20, 70, 200, 40, exports.cr_ui:isInBox(20, 70, 200, 40) and tocolor(21, 124, 165, 250) or tocolor(21, 124, 165, 200))
		dxDrawText("Karakter Oluştur [SPACE]", 35, 81, nil, nil, tocolor(255, 255, 255, 250), 1, fonts.font3)
		
		if exports.cr_ui:isInBox(20, 70, 200, 40) and getKeyState("mouse1") and click+500 <= getTickCount() then
			click = getTickCount()
...
addEventHandler("onClientClick", root,
    function(b, s)
        ...
        if s == "down" then
            ...
            if exports.cr_ui:isInBox(x,y,w,h) and page == "Login" then
                --outputChatBox("asd2")
            ...
            for k,v in pairs(textbars) do
                ...
                if exports.cr_ui:isInBox(x,y,w,h) then
...
function startCharReg()
   ...
   setTimer(
        function()
            text = "Los Santos'un tenha sokakları ile yüzleşmeye hazırmısın?"
            tStart = true
...
function drawnText()
    ...
    local font = exports.cr_fonts:getFont("sf-medium", 13)
...
function drawnBoxNationality()
    ...
    for i = 0, options - 1 do
        ...
        if i == 0 then
            btnText = 'Avrupa';
            btnIcon = "images/europe.png"--Cache["Carshop Textures"]["tick"]
            ir, ig, ib = 255,255,255
        elseif i == 1 then
            btnText = 'Amerika';
            btnIcon = "images/american.png"--Cache["Carshop Textures"]["x"]
            ir, ig, ib = 255,255,255
        elseif i == 2 then
            btnText = 'Asya';
            btnIcon = "images/asia.png"--Cache["Carshop Textures"]["x"]
            ir, ig, ib = 255,255,255
        elseif i == 3 then
            btnText = 'Afrika';
            btnIcon = "images/africa.png"--Cache["Carshop Textures"]["x"]
            ir, ig, ib = 255,255,255
        end

        if exports.cr_ui:isInBox(x + 5, y + 40 + (i*28), w - 10, 25) or se == i + 1 then
            if exports.cr_ui:isInBox(x + 5, y + 40 + (i*28), w - 10, 25) then
                selected = i
            end
...
function startCharRegPanel()
    ...
    font = exports['cr_fonts']:getFont("UbuntuBold", 10)
...
function drawnCharRegPanel()
    ...
    local x, y = 80 , 191*2
    cHover = nil
    if exports.cr_ui:isInBox(x,y,w,h) or cSelected == 2 then
        if exports.cr_ui:isInBox(x,y,w,h) then
            cHover = "woman"
        end
        
        local r,g,b = 210,49,49
        dxDrawImage(x,y,w,h, "images/female.png", 0,0,0, tocolor(r, g, b, alpha))
    else
        dxDrawImage(x,y,w,h, "images/female.png", 0,0,0, tocolor(255, 255, 255, alpha))
    end
    
    local w, h = 14, 30
    local x, y = 210 , 191*2
    if exports.cr_ui:isInBox(x,y,w,h) or cSelected == 1 then
        if exports.cr_ui:isInBox(x,y,w,h) then
            cHover = "man"
        end
        
        local r,g,b = 61,122,188
        dxDrawImage(x,y,w,h, "images/male.png", 0,0,0, tocolor(r, g, b, alpha))
    else
        dxDrawImage(x,y,w,h, "images/male.png", 0,0,0, tocolor(255, 255, 255, alpha))
    end
    ...
    local yuksek = 900
    local yanalum = 150
    if exports.cr_ui:isInBox(sx/2 - tWidth/2 - between - 25, sy - yuksek/2 - 25/2, 25, 25) then
        cHover = "arrowLeft"
        dxDrawImage(sx/2 - tWidth/2 - between - 25, sy - yuksek/2 - 25/2, 25, 25, "images/arrowLeft.png", 0,0,0, exports.cr_ui:getServerColor(1, alpha))
    else
        dxDrawImage(sx/2 - tWidth/2 - between - 25, sy - yuksek/2 - 25/2, 25, 25, "images/arrowLeft.png", 0,0,0, tocolor(255, 255, 255, alpha))
    end

    if exports.cr_ui:isInBox(sx/2 + tWidth/2 + between + yanalum, sy - yuksek/2 - 25/2, 25, 25) then
        cHover = "arrowRight"
        dxDrawImage(sx/2 + tWidth/2 + between + yanalum, sy - yuksek/2 - 25/2, 25, 25, "images/arrowRight.png", 0,0,0, exports.cr_ui:getServerColor(1, alpha))
    else
        dxDrawImage(sx/2 + tWidth/2 + between + yanalum, sy - yuksek/2 - 25/2, 25, 25, "images/arrowRight.png", 0,0,0, tocolor(255, 255, 255, alpha))
    end
    ...
    elseif exports.cr_ui:isInBox(sx - 20 - 177, sy - h/2 - 27/2, 177, 27) then
...
function startVideo(cmd)
    fadeCamera(false)
    text = "Sunucumuza hoşgeldin, iyi roller!"
    start = true
    startTick = getTickCount()
    addEventHandler("onClientRender", root, drawnText, true, "low-5") 
    setTimer(function() 
        text = "Crown Roleplay - discord.gg/crownrp"
        start = true
        startTick = getTickCount()
            setTimer(function() 
                start = false
...
function drawnLoadingScreen()
    local font = exports['cr_fonts']:getFont("sf-medium", 8)
...
function drawnTour()    
    ...
    local font = exports['cr_fonts']:getFont("UbuntuBold", 8)
    local font2 = exports['cr_fonts']:getFont("UbuntuRegular", 8)
    o = o + multipler
