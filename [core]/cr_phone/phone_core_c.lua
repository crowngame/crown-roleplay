...
slidingSpeed = 20
contactListLimit = {}
font1 = guiCreateFont (":cr_resources/cartwheel.otf", 12)
curEditPhoneNumber = ""
renderedDX = false
phoneSetupState = false
guifont = guiCreateFont(":cr_fonts/files/UbuntuRegular.ttf", 9)
...
wFont1 = dxCreateFont("fonts/rbold.ttf",11) 
...
function drawnPhone()
	...
	if phoneSetupState == 2 then
		...
		if settingPageActive or twitterActive then
			dxDrawImage(x, y, w, h, "images/iphone_black.png", 0, 0, 0, tocolor(255, 255, 255))
		else
		...
		dxDrawText(string.format("%02d:%02d", getRealTime().hour, getRealTime().minute), x+10, y+30, w+x-158, h, tocolor(255, 255, 255, 255), 1, exports.cr_fonts:getFont("UbuntuBold", 9), "center")
			...
			if guiGetVisible(v) == true and isElement(v) then
				if (tonumber(guiGetAlpha(v)) ~= 0) then
					if guiGetEnabled(v) == true then
						dxDrawRectangle(bx,by,bw,bh,tocolor(27, 27, 27,220))
					else
...
function drawHomeMidBlock()
	if isPhoneGUICreated() then
		...
		exports.cr_global:guiComboBoxAdjustHeight(cHotlines, 12)
...
function hidePhoneGUI()
	...
	if isPhoneGUICreated() then
		...
		setElementData(localPlayer, "cellphoneGUIState", "slidedOut")
	end
	closeConfirmBox()
	killSettingSounds()
	--exports.cr_logs:closeInfoBox()
end
...
function powerOnPhone()
	if isPhoneGUICreated() then
		...
		alphaTmp = nil
		powerOnPhone_logo = guiCreateStaticImage(30, 200, rawW, rawH, ":cr_ui/public/images/logos/solid.png", false, wPhoneMenu)
		guiSetProperty(powerOnPhone_logo, "ImageColours", exports.cr_ui:getServerColor(3))
...
function powerOffPhone()
	if isPhoneGUICreated() then
		...
		local rawW, rawH = 257*ratio, 120*ratio
		alphaTmp = nil
		powerOnPhone_logo = guiCreateStaticImage(30, 200, rawW, rawH, ":cr_ui/public/images/logo.png", false, wPhoneMenu)
		--guiSetAlpha(logo, alpha)
		powerOnPhone_text = guiCreateLabel(80, 180+rawH+5, rawW, rawH, "Starting up...0%", false, wPhoneMenu)
		--guiSetAlpha(text, alpha)
		guiSetEnabled(wPhoneMenu, false)
...
function playToggleSound()
	...
	playSound(":cr_resources/toggle.mp3")
end
...
addEventHandler("replacePhoneAnimation", root,
	function(player,state)
		if localPlayer == player then
			...
			exports["cr_animation"]:serverToAnim("sarp:phone", "phone_talk")
		end
	end
)