...
function getPhoneSettings(phone, doNotCreateNew) --powerOn, ringtone, isSecret, isInPhonebook, boughtById, boughtByName, boughtDate, sms_tone, tone_volume
	...
	local phoneSettings = mysql:query_fetch_assoc("SELECT *, `charactername`, DATE_FORMAT(`bought_date`,'%b %d %Y %h:%i %p') AS `bought_date` FROM `phones` LEFT JOIN `characters` ON `phones`.`boughtby` = `characters`.`id` WHERE `phonenumber`='" ..  (tostring(phone)) .. "' LIMIT 1")
	if not phoneSettings then
		if doNotCreateNew then
			return false
		else
			mysql:query_free("INSERT INTO `phones` SET `phonenumber`='" .. exports.cr_global:toSQL(phone) .. "'   ")
		end
	else
...
addEventHandler("onResourceStart", root, 
    function(startedRes)
        if getResourceName(startedRes) == "mysql" then
            connection = exports["cr_mysql"]:getConnection()
...
function powerOn(phone, state)
	...
	return triggerClientEvent(source, "phone:powerOn:response", source, mysql:query_free("UPDATE `phones` SET `turnedon`='" .. state .. "' WHERE `phonenumber`='" ..  (tostring(phone)) .. "'"), state)
end
...
function applyPhone(string, popOutOnPhoneCall)
	...
	items = exports["cr_items"]:getItems(source)
	phoneName = "iPhone 15"
	for index, row in ipairs(items) do
		if row[1] == 2 then
			phoneName = exports["cr_items"]:getItemName(2, row[2], row[5])
		end
	end
	if phoneName == "Cellphone" then phoneName = "iPhone 15" end
	if string == "phone_in" then
		if getElementData(source, "phone_anim") ~= "0" then
			...
			exports.cr_boneattach:attachElementToBone(phoneO[source], source, 12, 0, 0.04, 0.1, 0, 90, 90)
		...
		if getElementData(source, "cellphoneGUIStateSynced") ~= 1 then
			setElementData(source, "cellphoneGUIStateSynced", 1)
		end
		setElementData(source, "cellphoneGUIStateSynced", 1)
	elseif string == "phone_talk" then
		if getElementData(source, "phone_anim") ~= "0" then
			...
			setElementDimension(phoneO[source], getElementDimension(source))
			setElementInterior(phoneO[source], getElementInterior(source))
			exports.cr_boneattach:attachElementToBone(phoneO[element], source, 12, 0, 0.04, 0.1, 0, 90, 90)
	...
	elseif string == "phone_out" then
		...
		setElementData(source, "cellphoneGUIStateSynced", nil)
		if isElement(phoneO[source]) then
...
function destroyPhone(element)
	if canPlayerCall(element) then
		exports.cr_global:removeAnimation(element) 
	end
	if isElement(phoneO[element]) then
		exports.cr_boneattach:detachElementFromBone(phoneO[element])
		destroyElement(phoneO[element]) 
		phoneO[element] = nil
	end
end

addCommandHandler("setme", function(thePlayer) setElementData(thePlayer, "calling", false)
			setElementData(getLocalPlayer(), "tlfkulubesi", false)  end)
...
function callSomeone(thePlayer, commandName, phoneNumber, withNumber)
	...
	if (logged==1) then
		...
		if publicphone or exports.cr_global:hasItem(thePlayer, 2) then
			...
			elseif withNumber and withNumber > 10 then
				if exports.cr_global:hasItem(thePlayer, 2, tonumber(withNumber))  then
			...
			if not tonumber(phoneNumber) then
				local result = mysql:query_fetch_assoc("SELECT entryNumber FROM phone_contacts WHERE entryName='" ..  (tostring(phoneNumber)) .. "' AND phone='" ..  (tostring(outboundPhoneNumber))  .. "' LIMIT 1")
				if result then
			...
			if not publicphone then
				...
				local phoneSettings = mysql:query_fetch_assoc("SELECT * FROM `phones` WHERE `phonenumber`='" ..  (tostring(testNumber)) .. "'")
				if not phoneSettings then
					mysql:query_free("INSERT INTO `phones` (`phonenumber`) VALUES ('" ..   (tostring(testNumber))  .. "')")
			...
			if callerphoneIsTurnedOn == 0 then
				...
				elseif getElementData(thePlayer, "injuriedanimation") then
					...
					if isNumberAHotline(phoneNumber) then
						...
						setElementData(thePlayer, "calling", phoneNumber)
						setElementData(thePlayer, "callingwith", outboundPhoneNumber)	
					...
					-- Otherwise find a fool to answer it
						...
						if bankMoney >= 1 then
							if not exports.cr_donators:hasPlayerPerk(thePlayer, 6) and not setElementData(thePlayer, "bankmoney", tonumber(bankMoney) - 1) then
						...
						setElementData(thePlayer, "call.col", publicphone)
						setElementData(thePlayer, "calling", tonumber(phoneNumber))
						setElementData(thePlayer, "called", true)
						setElementData(thePlayer, "callingwith", outboundPhoneNumber)
						setElementData(foundElement, "calling", tonumber(outboundPhoneNumber))
						setElementData(foundElement, "called", nil)
						
						local reconning = getElementData(foundElement, "reconx")
						if not reconning then 
							if calledphoneRingTone ~= 0 then
								for _,nearbyPlayer in ipairs(exports.cr_global:getNearbyElements(foundElement, "player"), 10) do
									triggerClientEvent(nearbyPlayer, "startRinging", foundElement, 1, calledphoneRingTone)
								end
							end
							triggerEvent('sendAme', foundElement, " telefonu çalmaya başladı.")
						end
						...
						if (callerphoneIsSecretNumber == 1) or publicphone then
							...
							local callerIDQuery = mysql:query("SELECT `entryName` FROM `phone_contacts` WHERE `phone`='" ..   (phoneNumber)  .. "' AND `entryNumber`='" ..   (outboundPhoneNumber) .. "' LIMIT 1")
						...
						exports['cr_logs']:dbLog(thePlayer, 29, { thePlayer, "ph" .. tostring(realOutboundPhoneNumber), foundElement, "ph" .. tostring(phoneNumber) }, "**Starting call - " .. display .. "**") 
					end
				end
			end
		else
...
function makeFCall(thePlayer, commandName, phoneNumber)
	...
	if fPhone and factionPhone then
		num = string.format("%02d%02d", factionPhone, fPhone)
		if not (phoneNumber) then
			outputChatBox("KULLANIM: /" .. commandName .. " [Phone Number]", thePlayer, 255, 194, 14)
		else
	...
	for _, phoneNumber in ipairs(phoneNumbers) do
		...
		if found and foundElement and isElement(foundElement) then
			...
			if (phoneState==0) then
				setElementData(foundElement, "calling", nil)
				setElementData(foundElement, "called", nil)
				setElementData(foundElement, "call.col", nil)
			end
		end
	end
end
			...
			elseif (calling) then
				...
				if (phoneState==0) then
					...
					if not found then
						...
						setElementData(foundElement, "phonestate", 1, false)
						setElementData(foundElement, "called", nil, false)
						...
						exports['cr_logs']:dbLog(thePlayer, 29, { thePlayer, "ph" .. tostring(ownPhoneNo), foundElement, "ph" .. tostring(calling) }, "**Picked up phone**") 
					end
...
function phoneBook(thePlayer, commandName, partialNick)
	...
	if (logged==1) then
		if (exports.cr_global:hasItem(thePlayer, 7)) then
			if not (partialNick) then
				outputChatBox("KULLANIM: /phonebook [Partial Subscriber Name / Number]", thePlayer, 255, 194, 14)
			else
				triggerEvent('sendAme', thePlayer, "looks into their phonebook.")
				local result = mysql:query("SELECT `charactername`, `phonenumber` FROM `phones` LEFT JOIN `characters` ON `phones`.`boughtby`=`characters`.`id` WHERE `phonebook`=1 AND (`charactername` LIKE '%" ..  (partialNick) .. "%' OR `phonenumber` LIKE '%" ..  (partialNick) .. "%') AND `secretnumber` = 0 LIMIT 20")
	...
	if (logged==1) then
		if (exports.cr_global:hasItem(thePlayer, 2)) then
			...
			if not tonumber(number) then
				local result = mysql:query_fetch_assoc("SELECT entryNumber FROM phone_contacts WHERE entryName='" ..  (tostring(number)) .. "' AND phone='" ..  (tostring(phoneNumber))  .. "' LIMIT 1")
				if result then
					numberName = number
			...
			if not publicphone then
				local phoneSettings = mysql:query_fetch_assoc("SELECT * FROM `phones` WHERE `phonenumber`='" ..  (tostring(phoneNumber)) .. "'")
				if not phoneSettings then
					mysql:query_free("INSERT INTO `phones` (`phonenumber`) VALUES ('" ..   (tostring(phoneNumber))  .. "')")
					callerphoneIsSecretNumber = 0
					callerphoneIsTurnedOn = 1
					callerphoneRingTone = 1
					callerphonePhoneBook = 1
			...
			if not number or not (...) then
				outputChatBox("KULLANIM: /" .. commandName .. " [number] [message]", thePlayer, 255, 194, 14)
			elseif callerphoneIsTurnedOn == 0 then
				outputChatBox("Your phone is off.", thePlayer, 255, 0, 0)
			elseif getElementData(thePlayer, "injuriedanimation") then
				outputChatBox("You can't use your phone while knocked out.", thePlayer, 255, 0, 0)
			end
			--elseif exports.cr_global:hasMoney(thePlayer, 1) or exports.cr_donators:hasPlayerPerk(thePlayer, 6) then
				...
				if tonumber(number) == 5555 then
					--[[if not exports.cr_donators:hasPlayerPerk(thePlayer, 6) then
						exports.cr_global:takeMoney(thePlayer, 1)
					end]]
							...
							if isElement(thePlayer) then
								...
								if id then
									...
									if impounded then
										...
										if amount > 0 then
											outputChatBox("SMS from #5555 [#" .. phoneNumber .. "]: " .. amount .. " of your vehicles are impounded. Head over to the Impound to release them.", thePlayer, 120, 255, 80)
										else
											outputChatBox("SMS from #5555 [#" .. phoneNumber .. "]: None of your vehicles are impounded.", thePlayer, 120, 255, 80)
										end
									end
								end
							end
				...
				elseif tonumber(number) == 7332 then
					...
					if getElementData(thePlayer, "adminjailed") then
						...
						setTimer(
							function(thePlayer)
								if isElement(thePlayer) then
									--exports.cr_global:takeMoney(thePlayer, 3)
									
									local reconning2 = getElementData(thePlayer, "reconx")
									if not reconning2 then
										triggerEvent('sendAme', thePlayer, "receives a text message.")
									end
									outputChatBox("SMS from #7332 [#" .. phoneNumber .. "]: Thanks for your message. - Los Santos Network", thePlayer, 120, 255, 80)
								end
							end, 3000, 1, thePlayer
						)
						...
						exports.cr_global:giveMoney(theTeam, 3)
					
						for key, value in ipairs(teamMembers) do
							local hasItem, index, number, dbid = exports.cr_global:hasItem(value,2)
							if(hasItem)then
								...
								local message2 = call(getResourceFromName("cr_language"), "applyLanguage", thePlayer, value, call(getResourceFromName("cr_chat"), "trunklateText", value, message), language)
					...
					if target then
						...
						local message2 = call(getResourceFromName("cr_language"), "applyLanguage", thePlayer, target, message, language)
						...
						exports['cr_logs']:dbLog(thePlayer, 30, {thePlayer, "ph" .. tostring(phoneNumber), target, "ph" .. tostring(number)}, "[" .. languagename .. "] " ..message)
					else
						local reconning = getElementData(thePlayer, "reconx")
						if not reconning then
							triggerEvent('sendAme', thePlayer, "sends a text message.")
...
function togglePhone(thePlayer, commandName, phoneNumber)
	...
	if logged == 1 then
		if not phoneNumber then
			local foundPhone,_,foundPhoneNumber = exports.cr_global:hasItem(thePlayer, 2)
		...
		elseif tonumber(phoneNumber) < 10 then
			...
			if not (exports.cr_global:hasItem(thePlayer, 2, tonumber(phoneNumber))) then
		...
		if getElementData(thePlayer, "calling") then
			...
			mysql:query_free("UPDATE `phones` SET `turnedon`='" .. (1 - calledphoneIsTurnedOn)  .. "' WHERE `phonenumber`='" ..   (tostring(phoneNumber))  .. "'")
		end
	end
end
addCommandHandler("togglephone", togglePhone)

--[[I have no idea what you're trying to do here but this will not work anymore as I just updated the phone. Fix it yourself / Farid
for i = 1, 20 do
	addCommandHandler("sms" .. tostring(i), 
		function(thePlayer, commandName, number, ...)
			if i <= exports['cr_items']:countItems(thePlayer, 2) then
...
function setPhoneBook(thePlayer, commandName, phoneNumber, ...)
	...
	if logged == 1 then
		...
		if name then
			name = name:sub(1, 40)
			success = mysql:query_free("UPDATE `phones` SET `phonebook`='" ..  (name)  .. "' WHERE `phonenumber`='" ..   (tostring(phoneNumber))  .. "'")
			outputChatBox("You've set your phonebook entry to '" .. name .. "'.", thePlayer, 0, 255, 0)
		else
			success = mysql:query_free("UPDATE `phones` SET `phonebook`=NULL WHERE `phonenumber`='" ..   (tostring(phoneNumber))  .. "'")
		...
		for key, value in ipairs(exports.cr_pool:getPoolElementsByType("player")) do
			...
			if (logged==1) then
				if phoneNumber >= 10000 then -- players can only purchase 5 digit numbers
					...
					local foundPhone,_,foundPhoneNumber = exports.cr_global:hasItem(value, 2, tonumber(phoneNumber))
					if foundPhone then
						return true, value
					end
				elseif phoneNumber >= 1000 and exports.cr_global:hasItem(value, 2) then -- 4 digit number for factions (2 digits for faction, 2 digits for specific player)
...
function fetchFirstPhoneNumber(target)
	local foundPhone,_,foundPhoneNumber = exports.cr_global:hasItem(target, 2, tonumber(phoneNumber))
	return foundPhoneNumber
end

function setEDX(thePlayer, index, newvalue, sync, nosyncatall)
	return setElementData(thePlayer, index, newvalue, sync, nosyncatall)
end
...
function cleanUpOnePlayer(player)
	...
	setElementData(player, "cellphoneGUIStateSynced", nil)
end
...
function findToNumber(player,number)
	...
	for k,v in ipairs(getElementsByType("player")) do
		if getElementData(v,"loggedin") == 1 then
			if exports["cr_global"]:hasItem(v,2,number) then
				found = found + 1
				if found == 1 then
...
addCommandHandler("numaram", function(thePlayer)
	...
	if found then
		outputChatBox("[!]#FFFFFF Telefon Numaranız: " .. number, thePlayer, 0, 0, 255, true)
	else
