...
function requestPhoneGUI(itemValue, newSource)
	...
	local phoneSettings = mysql:query_fetch_assoc("SELECT * FROM `phone_settings` WHERE `phonenumber`='" ..  (tostring(ownCellnumber)) .. "'")
	if not phoneSettings then
		mysql:query_free("INSERT INTO `phone_settings` (`phonenumber`) VALUES ('" ..   (tostring(ownCellnumber))  .. "')")
	...
	local mQuery1 = mysql:query("SELECT `entryName`, `entryNumber` from `phone_contacts` WHERE `phone`='" ..   (ownCellnumber)  .. "'")
	while true do
...
function getContacts(fromNumber)
	...
	if fromNumber and tonumber(fromNumber) then
		local phoneSettings = mysql:query_fetch_assoc("SELECT * FROM `phones` WHERE `phonenumber`='" ..  (fromNumber) .. "' LIMIT 1")
		if not phoneSettings then
			mysql:query_free("INSERT INTO `phones` (`phonenumber`) VALUES ('" ..   (fromNumber)  .. "')")
			callerphoneIsSecretNumber = 0
			callerphoneIsTurnedOn = 1
			callerphoneRingTone = 1
			callerphonePhoneBook = 1
		...
		local mQuery1 = mysql:query("SELECT * from `phone_contacts` WHERE `phone`='" ..   (fromNumber)  .. "' ORDER BY `entryName` ")
		while true do
			local row = mysql:fetch_assoc(mQuery1)
			if not row then break end
		...
		mysql:free_result(mQuery1)
		limit = mysql:query_fetch_assoc("SELECT `contact_limit` FROM `phones` WHERE `phonenumber`='" ..  (fromNumber) .. "' LIMIT 1")
	...
	if (client) then
		...
		if name and number then
			if tonumber(number) then
				local result = mysql:query_free("DELETE FROM `phone_contacts` WHERE `phone`='" ..   (phoneBookPhone) .. "' AND `entryName`='" ..   (name)  .. "' AND `entryNumber`='" ..   (number)  .. "'")
				if result then
...
function saveCurrentRingtone(itemValue, phoneBookPhone)
	if client and itemValue then
		...
		local result = mysql:query_free("UPDATE `phones` SET `ringtone`='" ..   (itemValue) .. "' WHERE `phonenumber`='" ..  (phoneBookPhone) .. "'")
		if not result then
			outputChatBox("Error, please try it again.", client, 255,0,0)
			return
		end
