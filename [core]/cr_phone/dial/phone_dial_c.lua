...
function drawPhoneDial()
	if isPhoneGUICreated() then
		curEditPhoneNumber = ""
		ePhoneNumber = guiCreateEdit(30,55,204,25,"",false,wPhoneMenu)
		...
		bCall = guiCreateButton(30,90,102,30,"Ara",false,wPhoneMenu)
		guiSetFont (bCall, font1)
		bSMSDial = guiCreateButton(30+102,90,102,30,"SMS",false,wPhoneMenu)
...
function startDialing(from, to, popOutOnPhoneCall)
	if from and to and string.len(to) > 0 then
		...
		if not callingTo then
			if not tonumber(to) and to ~= "Özel" then
				return false
			end
			callingTo = {to, nil}
		end
		...
		local posY = 60+yoffset
		local margin = 30
		local lineH, lineW = margin, 200
		local lineW2 = 150

		if not font2 then
			font2 = guiCreateFont (":cr_resources/nametags0.ttf", 17)
		end
		...
		if popOutOnPhoneCall then
			...
			guiSetText(lCallingText, "Bağlanıldı!")
			startCounting()
			calledHotline = true
...
function finishPhoneCall(reason, isLock)
	killDialingSounds()
	killDialingTimers()
	...
	local reasonText = "Bitti"
	if reason == "out_of_service" then
		reasonText = "Aradığınız numaraya\nşu anda ulaşılamıyor"
	...
	if not calledHotline and callingCost > 0 then
		outputDebugString("[Telefon] takeCallCost / " .. getPlayerName(localPlayer) .. " / " .. actualCountedTime .. " / $" .. callingCost)
		...
		if action then
			...
			elseif action == "connected" then
				if data then
					killDialingTimers()
					killDialingSounds()
					local sound1 = playSound("sounds/viberate.mp3")
					table.insert(dialingSounds, sound1)
...
function drawRinging(phoneRinging)
	...
	wRing = guiCreateWindow(ringOffSetOut,y,w,h, "Telefonuna #" .. phoneRinging .. " numarasından gelen bir cevapsız arama var.", false)
    guiWindowSetMovable(wRing, false)
    guiWindowSetSizable(wRing, false)

    bRingingAnswer = guiCreateButton(10, 28, 117, 35, "Cevap ver", false, wRing)
    bRingingDecline = guiCreateButton(129, 28, 118, 35, "Kapat", false, wRing)
