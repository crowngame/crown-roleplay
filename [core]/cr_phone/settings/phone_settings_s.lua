function updatePhoneSetting(fromPhone, index, value)
	...
	if fromPhone and index and value then
		...
		return mysql:query_free("UPDATE `phones` SET `" .. index .. "`='" .. exports.cr_global:toSQL(value) .. "' WHERE `phonenumber`='" .. fromPhone .. "' ")
	end
end
...
function activatePrivateNumber(fromPhone, cost)
	...
	if fromPhone and cost then
		local took, reason = exports.cr_donators:takeGC(source, cost)
		if not took then
			outputChatBox("You have failed to activate this Premium Feature. Reason: " .. reason, source, 255, 0, 0)
			return false
		end
		exports.cr_donators:addPurchaseHistory(source, "Cellphone Private Number on #" .. fromPhone, -cost)
