...
function openContactDetails(name, number, yoffset)
	if isPhoneGUICreated() then
		...
		fName_contact_details = guiCreateEdit(30+lineW,50+yoffset,lineW2,20, theContact2.entryName or "", false, wPhoneMenu)
		...
		local bW, bH = 100, 30
		bUpdate_contact_details = guiCreateButton(margin+bW,posY,bW,bH, "Güncelle", false, wPhoneMenu)
		guiSetEnabled(bUpdate_contact_details, false)
		posY = posY + lineH
		
		bCall1_contact_details = guiCreateButton(margin,posY,bW,bH, "Ara", false, wPhoneMenu)
		bSMS_contact_details = guiCreateButton(margin+bW+2,posY,bW,bH, "Mesaj Gönder", false, wPhoneMenu)
