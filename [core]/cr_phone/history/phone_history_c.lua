...
function drawHistory(xoffset, yoffset)
	...
	if not requesting_history and not histories[phone] then
		...
		wHistory = guiCreateScrollPane(30+xoffset, 55+yoffset, 221, 370, false, wPhoneMenu)
		guiCreateLabel(0.5, 0.5, 1, 0.5, "Yükleniyor...", true, wHistory)
...
function refreshHistory(newHistory, xoffset, yoffset, fromServer)
	...
	if #histories[tonumber(phone)] > 0 then
		for i = 1, #histories[tonumber(phone)] do 
			...
			guiSetAlpha(guiCreateStaticImage(10+xoffset, 50+yoffset, 200, 1, ":cr_admin/components/whitedot.jpg", false, wHistory), 0.1)
		    yoffset = yoffset + 40 
		end
...
function analyzeHistory(his)
	...
	--outputChatBox("exports.cr_datetime:formatTimeInterval(datesec) - " .. exports.cr_datetime:formatTimeInterval(datesec))
	return history_name, history_number, exports.cr_datetime:formatTimeInterval(datesec), flow, private
end
