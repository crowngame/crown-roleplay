...
function sendSMS(from, to, content, private)
    ...
    mysql:query_insert_free("INSERT INTO `phone_sms` SET `from`='" .. from .. "', `to`='" .. to .. "', `content`='" .. exports.cr_global:toSQL(content) .. "', private=" .. private)
    if not isNumberAHotline(to) then
        ...
        if not t_powerOn then --not existed
            ...
            mysql:query_insert_free("INSERT INTO `phone_sms` SET `from`='" .. to .. "', `to`='" .. from .. "', `content`='" .. exports.cr_global:toSQL(notExisted) .. "' ")
...
function startRingingSMS(fromPhone, smsTone, volume)
    ...
    for _,nearbyPlayer in ipairs(exports.cr_global:getNearbyElements(source, "player"), 10) do
        triggerClientEvent(nearbyPlayer, "startRinging", source, 2, smsTone, volume)
    end

    outputChatBox("[!]#FFFFFF (" .. fromPhone .. ") numarası size yeni bir mesaj gönderdi!", source, 0, 0, 255, true)
