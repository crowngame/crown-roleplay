...
function drawAllSMSThreads(xoffset, yoffset)
	...
	wSMS = guiCreateScrollPane(30+xoffset, 55+yoffset, 230, 370, false, wPhoneMenu)
	...
	if sortedThreads[phone] and smsThreadInitiated[phone] then
		...
		if #sortedThreads[phone] > 0 then
			...
			for i = 1, #sortedThreads[phone] do
				...
				if thread then
					...
					guiSetAlpha(guiCreateStaticImage(10+xoffset, 50+yoffset, 200, 1, ":cr_admin/components/whitedot.jpg", false, wSMS), 0.1)
				    yoffset = yoffset + 40 
				else
					guiCreateLabel(0.5, 0.3, 1, 0.5, "Hiç mesaj almamışsın.", true, wSMS)
					isEmpty = true
					break
				end
			end
		...
		guiCreateLabel(0.5, 0.3, 1, 0.5, "Yükleniyor...", true, wSMS)
	end
end
...
--	end
	...
	if date2 then
		local date = guiCreateLabel(xoffset+margin, xoffset+margin*2+newContentH, boxW-margin*2, boxH-margin*2, exports.cr_datetime:formatTimeInterval(date), false, body)
...
function drawOneSMSThread(contact, xoffset, yoffset)
	...
	local w,h = 230, 350
	wSMS = guiCreateScrollPane(30+xoffset, 55+yoffset, w, h , false, wPhoneMenu)
...
function drawSMSComposer(xoffset, yoffset)
	if isPhoneGUICreated() then
		...
		limit = 120
		wSMSComposer = guiCreateScrollPane(30+xoffset, 80+yoffset, 210, 65, false, wPhoneMenu)
