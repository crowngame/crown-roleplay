...
function SAN_newAdvert(message, sourceName, sourceElement, theCost, outboundPhoneNumber)
	...
	exports.cr_global:sendMessageToSupporters("[ADVERT] ID#" .. advertID .. " (" .. advertMsg .. ") created by " .. sourceName .. ".")
	...
	for adIndex, adTable in ipairs (ads) do
		...
		elseif adTable[AD_FROZEN] then
			...
			if (currentTime - adTable [AD_EDITTIME]) > 5 then
				table.insert(removeIDs, adIndex)
				exports.cr_global:sendMessageToSupporters("[ADS] Removing ad " .. adIndex .. " due being 10 minutes frozen. (`" .. adTable[AD_MESSAGE] .. "`, `" .. adTable[AD_SENDERNAME] .. "`)")
			end
		elseif (not didAirSomething) and not (adTable[AD_EDITTIME] == adTable[AD_CREATIONTIME]) then
			...
			exports.cr_logs:logMessage("ADVERT: " .. adTable[AD_MESSAGE] .. " ((" ..  adTable[AD_SENDERNAME] .. "))", 2)
			for key, value in ipairs(exports.cr_pool:getPoolElementsByType("player")) do
				if getElementData(value, "loggedin")==1 then
					local hasTogAd, togAdState = exports.cr_donators:hasPlayerPerk(value, 2)
					if hasTogAd and tonumber(togAdState) == 1 then
						--donothing
					else
						if exports.cr_integration:isPlayerHelper(value) or exports.cr_integration:isPlayerTrialAdmin(value) then
...
function SAN_freezeAdvert(thePlayer, commandHandler, advertID)
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerHelper(thePlayer) then
		if not advertID or not tonumber(advertID) then
			outputChatBox("KULLANIM: /" .. commandHandler .. " <advertID>", thePlayer)
			return
		end
		...
		exports.cr_global:sendMessageToSupporters("[AD] " .. getPlayerName(thePlayer) .. " froze advert " .. advertID .. ".")
	end
end
...
function SAN_unfreezeAdvert(thePlayer, commandHandler, advertID)
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerHelper(thePlayer) then
		...
		ads[advertID][AD_EDITTIME] = getRealTime().timestamp
		exports.cr_global:sendMessageToSupporters("[AD] " .. getPlayerName(thePlayer) .. " unfroze advert " .. advertID .. ".")
...
function SAN_deleteAdvert(thePlayer, commandHandler, advertID)
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerHelper(thePlayer) then
		...
		exports.cr_global:sendMessageToSupporters("[AD] " .. getPlayerName(thePlayer) .. " marked advert " .. advertID .. " as aired.")
	end
end
