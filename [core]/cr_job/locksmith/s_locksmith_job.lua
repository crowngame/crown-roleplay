	...
	if (logged==1) then
		if(getElementData(thePlayer,"job")==6)then
			if not (keyType) or not (keyID) then
				outputChatBox("KULLANIM: /" .. commandName .. " [Key Type: 1=House 2=Business 3=Vehicle] [Key ID]", thePlayer, 255, 194, 14)
			else
				if not exports.cr_global:hasMoney(thePlayer, 25) then
					...
					local success = exports.cr_global:hasItem(thePlayer, tonumber(itemID), tonumber(keyID))
					if(success)then -- does the player have the key?
						exports.cr_global:giveItem(thePlayer, tonumber(itemID), tonumber(keyID)) -- create a ket for the locksmith.
						exports.cr_global:takeMoney(thePlayer, 25) -- take the cost of making the key from the locksmith.
						outputChatBox("You have duplicated a " ..  keyname  .. " " ..  keyID  .. " at a cost of $15.", thePlayer)
						exports.cr_global:sendLocalMeAction(thePlayer,"duplicates a " ..  keyname  .. ".")
					else
