...
local jessie = createPed(141, 1533.2705078125, 1487.421875, 16.953544616699)
setElementDimension(jessie, 2695)
setElementInterior(jessie , 21)
...
function showEmploymentWindow()
	...
	-- Kamyon Soforu
	local row = guiGridListAddRow(jobList)
	guiGridListSetItemText(jobList, row, column, "Kamyon Soforu", false, false) -- Disabled, added mechanic faction type
	...
	-- İnşaat
	local row = guiGridListAddRow(jobList)
	guiGridListSetItemText(jobList, row, column, "Madenci", false, false)
	
	-- İnşaat
	local row = guiGridListAddRow(jobList)
	guiGridListSetItemText(jobList, row, column, "Çöp Şöförü", false, false)
...
function acceptJob(button, state)
	if (button=="left") then
		...
		elseif (job>0) then
			...
			if (jobtext=="Delivery Driver" or jobtext=="Taxi Driver" or jobtext=="Bus Driver" or jobtext=="Kamyon Soforu") then  -- Driving job, requires the license
			...
			if (jobtext=="Delivery Driver") then
				exports["cr_job-trucker"]:displayTruckerJob()
				job = 1
			...
			elseif (jobtext=="Locksmith") then
				displayLocksmithJob()
				job = 6
			elseif (jobtext=="Kamyon Soforu") then
				displayTruck1Job()
				job = 8
			elseif (jobtext=="Madenci") then
				--displayMinerJob()
				job = 9
			elseif (jobtext=="Çöp Şöförü") then
				--displayMinerJob()
				job = 16	
