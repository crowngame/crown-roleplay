...
setElementData(badguy2, "name", "Ricardo Diablo")
...
function cigarJobDisplayGUI(thePlayer)
	...
	if (ftype) and (ftype == 0) or (ftype == 1) then
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "Ricardo Diablo fısıldar: Hey, elimde bir iş var. Ne dersin, ha?", 255, 255, 255, 3, {}, true)
		cigarAcceptGUI(thePlayer)
		return
	else
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "Ricardo Diablo: Seninle bir işim yok. Derhal toz ol buradan.", 255, 255, 255, 10, {}, true)
		return
	end
end
...
function cigarAcceptGUI(thePlayer)
	...
	addEventHandler("onClientGUIClick", acceptBtn, 
		function()
			...
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "" .. getPlayerName(thePlayer):gsub("_", " ") .. ": " .. AcceptCigar[math.random(#AcceptCigar)], 255, 255, 255, 3, {}, true)
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "Ricardo Diablo: Yandaki vanlardan birini alarak işe başla, araçlar yüklü ve yola çıkmaya hazır. Bol şanslar, ahbap.", 255, 255, 255, 3, {}, true)
			setTimer(function() 
				----exports.cr_hud:sendBottomNotification(getLocalPlayer(), "Sigara Kaçakçılığı", "Yandaki beyaz kamyonlardan birini alıp, /sigara basla yazarak işe başlayabilirsiniz!")
				outputChatBox("[!]#FFFFFF Yandaki kamyonlardan birini alıp, /sigara basla yazarak işe başlayabilirsiniz!", 0, 0, 255, true)
			end, 500, 1)
			return	
		end
	)
	...
	addEventHandler("onClientGUIClick", cancelBtn, 
		function()
			destroyElement(jobWindow)
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "" .. getPlayerName(thePlayer):gsub("_", " ") .. ": " .. RejectCigar[math.random(#RejectCigar)], 255, 255, 255, 3, {}, true)
			return	
		end
	)
end
...
function cstartJob(cmd, arg)
	if arg == "basla" then
		if not cigarBlip then --not routeMarker then
			...
			outputChatBox("[!]#FFFFFF Zaten bir sefere başladınız!", 255, 0, 0, true)
		end
	end
end
				...
				for _, marker in ipairs(cigarOldMarker) do
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							...
							outputChatBox("[!]#FFFFFF Aracınıza yeni mallar yükleniyor, lütfen bekleyin.", 0, 0, 255, true)
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									outputChatBox("[!]#FFFFFF Aracınıza yeni mallar yüklenmiştir.", 0, 255, 0, true)
						...
						elseif marker[2] == true and marker[3] == false then
							...
							setElementFrozen(hitPlayer, true)
							setElementFrozen(hitVehicle, true)
							outputChatBox("[!]#FFFFFF Aracınızdaki mallar indiriliyor, lütfen bekleyin.", 0, 0, 255, true)
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									outputChatBox("[!]#FFFFFF Aracınızdaki mallar indirilmiştir. Devam edebilirsiniz.", 0, 255, 0, true)
									setElementFrozen(hitVehicle, false)
									setElementFrozen(thePlayer, false)
									cupdateRoutes()
								end, 1000, 1, hitPlayer, hitVehicle, source
...
function trashAntiYabanci(thePlayer, seat, door) 
	...
	if vehicleModel == 459 and vehicleJob == 8 then
		...
		elseif thePlayer == getLocalPlayer() and playerJob ~= 8 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("[!]#FFFFFF Bu araca binmek için Sigara Şoförlüğü mesleğinde olmanız gerekmektedir.", 255, 0, 0, true)
		cancelEvent()
		end
	end
end
