local theguy = createPed(16, 2281.1201, -2365.0683, 13.5468, 320, true)
setElementData(theguy, "talk", 1)
setElementData(theguy, "name", "James Caviezel")
setElementFrozen(theguy, true)

function truckJobDisplayGUI()
	local carlicense = getElementData(getLocalPlayer(), "license.car")
	
	if (carlicense == 1) then
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "James Caviezel: Müracaatınızda işi almanıza bir engel bulunamamıştır.", 255, 255, 255, 3, {}, true)
		truckAcceptGUI(getLocalPlayer())
	else
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "James Caviezel: Mesleği yapabilmeniz için öncelikle ehliyet sahibi olmalısınız.", 255, 255, 255, 10, {}, true)
		return
	end
end
addEvent("trucker:displayJob", true)
addEventHandler("trucker:displayJob", getRootElement(), truckJobDisplayGUI)

function truckAcceptGUI(thePlayer)
	...
	addEventHandler("onClientGUIClick", acceptBtn, 
		function()
			...
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "James Caviezel: Yandaki tırlardan birini alarak işe başla, tırın dorsesini almayı unutma.", 255, 255, 255, 3, {}, true)
			return	
		end
	)
	...
	if arg == "basla" then
		if not truckBlip then
			...
			if vehModel == jobVehicle then
				updateTruckRoutes()
				truckBlip = createBlip(2289.0332, -2368.7841, 13.0321, 0, 3, 255, 0, 0, 255)
				addEventHandler("onClientMarkerHit", resourceRoot, truckRoutesMarkerHit)
			end
		else
			--exports.cr_hud:sendBottomNotification(getLocalPlayer(), "Tır Şoförlüğü", "Zaten bir sefere başladın.")
		end
	end
end
addCommandHandler("tir", tstartJob)
	...
	for i,v in ipairs(truckRoutes) do
		if i == truckMarker then
			...
			elseif not v[4] == true then
				trouteMarker = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 0, 0, 255, getLocalPlayer())
				setElementPosition(truckBlip, v[1], v[2], v[3])
				setBlipColor(truckBlip, 255, 0, 0, 255)
				table.insert(truckOldMarker, { trouteMarker, false })
			elseif v[4] == true and v[5] == true then 
				tfMarker = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 255, 0, 255, getLocalPlayer())
				destroyElement(truckBlip)
				table.insert(truckOldMarker, { tfMarker, true, true })	
			elseif v[4] == true then
				tpMarker = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 255, 0, 255, getLocalPlayer())
				setElementPosition(truckBlip, v[1], v[2], v[3])
				setBlipColor(truckBlip, 255, 255, 0, 255)
				table.insert(truckOldMarker, { tpMarker, true, false })			
			end
		end
	end
end
					...
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							if getVehicleTowedByVehicle(hitVehicle) then
								local hitVehicle = getPedOccupiedVehicle(hitPlayer)
								truckMarker = 0
								--exports.cr_hud:sendBottomNotification(hitPlayer, "Tır Şoförlüğü", "Bu turdan 420$ kazandınız.")
							...
							if getVehicleTowedByVehicle(hitVehicle) then
								...
								setTimer(
									function(thePlayer, hitVehicle, hitMarker)
										...
										local trailer = getVehicleTowedByVehicle(hitVehicle)
										--local attachedElements = getAttachedElements (hitVehicle)
										detachTrailerFromVehicle(hitVehicle)
										triggerServerEvent("trucker:destroyTrailer", hitPlayer, trailer)
										triggerServerEvent("trucker:pay", hitPlayer, hitPlayer)
										--outputChatBox("[!]#FFFFFF Dorseniz indirilmiş ve paranız ödenmiştir ($240), geri dönebilirsiniz.", 0, 255, 0, true)
										--exports.cr_hud:sendBottomNotification(hitPlayer, "Tır Şoförlüğü", "Dorseniz indirilmiş ve paranız ödenmiştir (420$), geri dönebilirsiniz.")
...
addCommandHandler("tir", loadTrailer)

function tcancelJob(trailer)
	...
	if pedVeh then
		if pedVehModel == 515 then
			...
			if isElement(trailer) then
				triggerServerEvent("trucker:destroyTrailer", getLocalPlayer(), trailer)
			end
...
function trashAntiYabanci(thePlayer, seat, door) 
	...
	if vehicleModel == 515 and vehicleJob == 10 then
		...
		elseif thePlayer == getLocalPlayer() and playerJob ~= 10 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("[!]#FFFFFF Bu araca binmek için tır Şoförlüğü mesleğinde olmanız gerekmektedir.", 255, 0, 0, true)
		cancelEvent()
		end
	end
end
...
function tcantExit(thePlayer, seat)
...
addEventHandler("onClientVehicleStartExit", getRootElement(), tcantExit)