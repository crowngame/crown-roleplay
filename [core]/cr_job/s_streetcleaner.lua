...
function initiateCleanerJob(thePlayer)
	...
	exports.cr_pool:allocateElement(blip)
...
function startCleaningMission(thePlayer)
	if (isElement(thePlayer)) then
		...
		exports.cr_pool:allocateElement(vehicle)
		...
		exports.cr_pool:allocateElement(blip2)
		exports.cr_pool:allocateElement(marker2)
		...
		exports.cr_pool:allocateElement(colsphere)
	...
	if (isPedInVehicle (thePlayer)) then
		...
		if tonumber(vehid) == 574 then
			...
			if tonumber(max_number) == tonumber(m_number) then
				...
				exports.cr_pool:allocateElement(marker3)
				exports.cr_pool:allocateElement(blip3)
...
function FinalCheckpoints(thePlayer)
	if (isPedInVehicle (thePlayer)) then
		...
		if tonumber(vehid) == 574 then
			...
			exports.cr_pool:allocateElement(marker4)
			exports.cr_pool:allocateElement(blip4)
	...
	if (isElement(thePlayer)) then
		if (isPedInVehicle (thePlayer)) then
			...
			if tonumber(vehid) == 574 then
				...
				setElementData(vehicle, "fuel")
				setElementData(vehicle, "dbid")
				
				if (isElement(vehicle)) then
					destroyElement(vehicle)
					vehicle = nil
					setElementData(thePlayer, "cleaner.vehicle")
					...
					exports.cr_global:giveMoney(thePlayer, 300)
...
function quit()
	...
	setElementData(source, "cleaner.vehicle")
end
