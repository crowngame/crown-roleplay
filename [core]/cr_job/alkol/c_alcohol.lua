...
setElementData(badguy, "name", "Antonio Banderas")
...
function jobDisplayGUI(thePlayer)
	...
	if (ftype) and (ftype < 2) then
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "Antonio Banderas fısıldar: Güzel, işe girmek istiyorsun demek.", 255, 255, 255, 3, {}, true)
		acceptGUI(thePlayer)
		return
	else
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "Antonio Banderas: Bu mesleği yapabilmek için bir mafya veya çete üyesi olman gerek ahbap.", 255, 255, 255, 10, {}, true)
		return
	end
end
...
function acceptGUI(thePlayer)
	...
	addEventHandler("onClientGUIClick", acceptBtn, 
		function()
			...
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "" .. getPlayerName(thePlayer):gsub("_", " ") .. ": " .. Accept[math.random(#Accept)], 255, 255, 255, 3, {}, true)
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "Antonio Banderas: Yandaki kamyonlardan birini alarak işe başla, kamyonlar yüklü ve yola çıkmaya hazır. Bol şanslar, ahbap.", 255, 255, 255, 3, {}, true)
	...
	addEventHandler("onClientGUIClick", cancelBtn, 
		function()
			destroyElement(jobWindow)
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "" .. getPlayerName(thePlayer):gsub("_", " ") .. ": " .. kabulEtmeMsg[math.random(#kabulEtmeMsg)], 255, 255, 255, 3, {}, true)
			return	
		end
	)
end
...
function startJob(cmd, arg)
	if arg == "basla" then
		if not alcoholBlip then--not routeMarker then
			...
			outputChatBox("[!]#FFFFFF Zaten Alkol Taşımacılığı işine başlamışsınız.", 255, 0, 0, true)
		end
	end
end
				...
				for _, marker in ipairs(OldMarker) do
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							...
							outputChatBox("[!]#FFFFFF Aracınıza yeni mallar yükleniyor, lütfen bekleyiniz.", 0, 0, 255, true)
							setTimer(
...
function cancelJob()
	...
	if pedVeh then
		if pedVehModel == 456 then
			...
			if isEventHandlerAdded ("onClientMarkerHit", resourceRoot, RoutesMarkerHit) then
				removeEventHandler("onClientMarkerHit", resourceRoot, RoutesMarkerHit)
			end 
			if isEventHandlerAdded("onClientVehicleStartEnter", getRootElement(), ickiAntiYabanci) then
...
function trashAntiYabanci(thePlayer, seat, door) 
	...
	if vehicleModel == 456 and vehicleJob == 9 then
		if thePlayer == getLocalPlayer() and seat ~= 0 then
			cancelEvent()
			outputChatBox("[!]#FFFFFF Meslek aracına yolcu olarak binemezsiniz.", 255, 0, 0, true)
		elseif thePlayer == getLocalPlayer() and playerJob ~= 9 then
			cancelEvent()
			outputChatBox("[!]#FFFFFF Bu araca binmek için Alkol Taşımacılığı mesleğinde olmanız gerekmektedir.", 255, 0, 0, true)
		end
	end
end
