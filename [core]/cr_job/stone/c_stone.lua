local stoneguy = createPed(16, 2336.79295, -2079.7926, 13.5537, 85, true)
setElementData(stoneguy, "talk", 1)
setElementData(stoneguy, "name", "Ron Perlman")
setElementFrozen(stoneguy, true)

function stoneJobDisplayGUI()
	local carlicense = getElementData(getLocalPlayer(), "license.car")
	
	if (carlicense == 1) then
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "Ron Perlman: Müracaatınızda işi almanıza bir engel bulunamamıştır.", 255, 255, 255, 3, {}, true)
		stoneAcceptGUI(getLocalPlayer())
		return
	else
		triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "Ron Perlman: Mesleği yapabilmeniz için öncelikle ehliyet sahibi olmalısınız.", 255, 255, 255, 10, {}, true)
		return
	end
end
addEvent("stone:displayJob", true)
addEventHandler("stone:displayJob", getRootElement(), stoneJobDisplayGUI)

function stoneAcceptGUI(thePlayer)
	local screenW, screenH = guiGetScreenSize()
	local jobWindow = guiCreateWindow((screenW - 308) / 2, (screenH - 102) / 2, 308, 102, "Meslek Görüntüle: Beton Taşımacılığı", false)
	...
	addEventHandler("onClientGUIClick", acceptBtn, 
		function()
			...
			triggerServerEvent("acceptJob", getLocalPlayer(), 14)
			triggerServerEvent("sendLocalText", getLocalPlayer(), getLocalPlayer(), "Ron Perlman: Yandaki kamyonlardan birini alarak işe başla, dikkatli olmayı unutma.", 255, 255, 255, 3, {}, true)
			--setTimer(function() --exports.cr_hud:sendBottomNotification(getLocalPlayer(), "Beton Taşımacılığı", "Yandaki kamyonlardan birini alıp, /beton basla yazarak işe başlayabilirsiniz!") end, 500, 1)
			return	
		end
	)
...
function sstartJob(cmd, arg)
	if arg == "basla" then
		if not stoneBlip then
			local veh = getPedOccupiedVehicle(getLocalPlayer())
			local vehModel = getElementModel(veh)
			local jobVehicle = 524
			if veh and getVehicleController(veh) == getLocalPlayer() then
			if vehModel == jobVehicle then
				updatestoneRoutes()
				stoneBlip = createBlip(2342.4902, -2051.8769, 14.3413, 0, 3, 255, 0, 0, 255)
				addEventHandler("onClientMarkerHit", resourceRoot, stoneRoutesMarkerHit)
			end
				else
		--exports.cr_hud:sendBottomNotification(localPlayer, "Beton Taşımacılığı", "Beton seferine başlamak için meslek aracında bulunmalısınız. (Beton)")
		end
					else
			--exports.cr_hud:sendBottomNotification(getLocalPlayer(), "Beton Taşımacılığı", "Zaten bir sefere başladınız.")
	end
	end
end
addCommandHandler("beton", sstartJob)


function updatestoneRoutes()
	stoneMarker = stoneMarker + 1
	for i,v in ipairs(stoneRoutes) do
		if i == stoneMarker then
			if not v[4] == true then
				srouteMarker = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 0, 0, 255, getLocalPlayer())
				if stoneBlip then
					setElementPosition(stoneBlip, v[1], v[2], v[3])
					setBlipColor(stoneBlip, 255, 0, 0, 255)
				end
				table.insert(stoneOldMarker, { srouteMarker, false })
			elseif v[4] == true and v[5] == true then 
				sfMarker = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 255, 0, 255, getLocalPlayer())
				destroyElement(stoneBlip)
				table.insert(stoneOldMarker, { sfMarker, true, true })	
			elseif v[4] == true then
				spMarker = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 255, 0, 255, getLocalPlayer())
				setElementPosition(stoneBlip, v[1], v[2], v[3])
				setBlipColor(stoneBlip, 255, 255, 0, 255)
				table.insert(stoneOldMarker, { spMarker, true, false })			
			end
		end
	end
end

function stoneRoutesMarkerHit(hitPlayer, matchingDimension)
	if hitPlayer == getLocalPlayer() then
		...
		if hitVehicle then
			...
			if hitVehicleModel == 524 then
				for _, marker in ipairs(stoneOldMarker) do
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							...
							stoneMarker = 0
							triggerServerEvent("stone:pay", hitPlayer, hitPlayer)
							--exports.cr_hud:sendBottomNotification(hitPlayer, "Beton Şoförlüğü", "Aracınıza yeni mallar yükleniyor, lütfen bekleyiniz.")
							--outputChatBox("[!]#FFFFFF Aracınıza yeni mallar yükleniyor, lütfen bekleyiniz. Eğer devam etmek istemiyorsanız, /scancelJob yazınız.", 0, 0, 255, true)
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									...
									--exports.cr_hud:sendBottomNotification(thePlayer, "Beton Şoförlüğü", "Aracınıza yeni mallar yüklenmiştir, geri dönebilirsiniz.")
						...
						elseif marker[2] == true and marker[3] == false then
							...
							--exports.cr_hud:sendBottomNotification(hitPlayer, "Beton Şoförlüğü", "Aracınızdaki mallar indiriliyor, lütfen bekleyiniz.")
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									--outputChatBox("[!]#FFFFFF Aracınızdaki mallar indirilmiştir, geri dönebilirsiniz.", 0, 255, 0, true)
									--exports.cr_hud:sendBottomNotification(thePlayer, "Beton Şoförlüğü", "Aracınızdaki mallar indirilmiştir, geri dönebilirsiniz.")
									setElementFrozen(hitVehicle, false)
									setElementFrozen(thePlayer, false)
									toggleAllControls(true)
									updatestoneRoutes()
...
function scancelJob()
	...
	if pedVeh then
		if pedVehModel == 524 then
			...
			for i,v in ipairs(stoneOldMarker) do
				destroyElement(v[1])
			end
			stoneOldMarker = {}
			stoneMarker = 0
			...
			triggerServerEvent("stone:exitVeh", getLocalPlayer(), getLocalPlayer())
			removeEventHandler("onClientMarkerHit", resourceRoot, stoneRoutesMarkerHit)
			removeEventHandler("onClientVehicleStartEnter", getRootElement(), scantJacked)
			setTimer(function() exports.cr_global:fadeFromBlack() end, 2000, 1)
		end
	end
end

function scantJacked(thePlayer, seat, door) 
	...
	if vehicleModel == 524 and vehicleJob == 14 then
		if thePlayer == getLocalPlayer() and seat ~= 0 then
			...
			setElementFrozen(thePlayer, false)
			--exports.cr_hud:sendBottomNotification(thePlayer, "Beton Taşımacılığı", "Meslek aracına binemezsiniz.")
		elseif thePlayer == getLocalPlayer() and playerJob ~= 14 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			--exports.cr_hud:sendBottomNotification(thePlayer, "Beton Taşımacılığı", "Bu araca binmek için Beton Taşımacılığı mesleğinde olmanız gerekmektedir.")
		end
	end
end
addEventHandler("onClientVehicleStartEnter", getRootElement(), scantJacked)

function scantExit(thePlayer, seat)
...
addEventHandler("onClientVehicleStartExit", getRootElement(), scantExit)