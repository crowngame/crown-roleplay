...
function serviceVehicle(veh)
	if (veh) then
		...
		if not exports.cr_global:takeMoney(source, mechcost) then
			...
			exports.cr_global:sendLocalMeAction(source, "services the vehicle, patching it up a bit.")
			exports.cr_logs:dbLog(source, 31, {  veh }, "REPAIR QUICK-SERVICE")
		end
	else
...
function changeTyre(veh, wheelNumber)
	if (veh) then
		...
		if not exports.cr_global:takeMoney(source, mechcost) then
			...
			exports.cr_logs:dbLog(source, 31, {  veh }, "REPAIR TIRESWAP")
			exports.cr_global:sendLocalMeAction(source, "replaces the vehicle's tyre.")
		end
	end
end
	...
	if (veh) then
		...
		if not exports.cr_global:takeMoney(source, mechcost) then
			...
			if setVehiclePaintjob(veh, paintjob) then
				...
				exports.cr_logs:logMessage("[/changePaintJob] " .. getPlayerName(source)  .. " / " ..  getPlayerIP(source)   .. " OR " .. getPlayerName(client)   .. " / " ..  getPlayerIP(client)   .. " changed vehicle " .. getElementData(veh, "dbid") .. " their colors to " .. col1 .. "-" .. col2, 29)
				exports.cr_global:sendLocalMeAction(source, "repaints the vehicle.")
				exports.cr_logs:dbLog(source, 6, {  veh }, "MODDING PAINTJOB " ..  paintjob)
				exports['cr_save']:saveVehicleMods(veh)
			else
	...
	if (veh) then
		...
		if not exports.cr_global:takeMoney(source, mechcost) then
			...
			if setVehicleHeadLightColor (veh, color1, color2, color3) then
				exports.cr_logs:logMessage("[/changeHeadlights] " .. getPlayerName(source)  .. " / " ..  getPlayerIP(source)   .. " OR " .. getPlayerName(client)   .. " / " ..  getPlayerIP(client)   .. " changed vehicle " .. getElementData(veh, "dbid") .. " their headlight colors to " .. color1 .. "-" .. color2 .. "-" .. color3, 29)
				exports.cr_global:sendLocalMeAction(source, "replaces the vehicles headlights.")
				setElementData(veh, "headlightcolors", {color1, color2, color3}, true)
				exports['cr_save']:saveVehicleMods(veh)
				exports.cr_logs:dbLog(source, 6, {  veh }, "MODDING HEADLIGHT " ..  color1 .. " " .. color2 .. " " .. color3)
			else
...
function changeVehicleUpgrade(veh, upgrade)
	if (veh) then
		...
		if exports.cr_global:hasItem(source, 114, upgrade) then
			mechcost = 0
			item = true
		end
		
		if not item and not exports.cr_global:hasMoney(source, mechcost) then
			...
			if addVehicleUpgrade(veh, upgrade) then
				if item then
					exports.cr_global:takeItem(source, 114, upgrade)
				else
					exports.cr_global:takeMoney(source, mechcost)
				end
				exports.cr_logs:logMessage("[changeVehicleUpgrade] " .. getPlayerName(source)  .. "/ " .. getPlayerIP(source)  .. " OR " .. getPlayerName(client)   .. "/ " .. getPlayerIP(client)  .. "  changed vehicle " .. getElementData(veh, "dbid") .. ": added " .. name .. " to the vehicle.", 29)
				exports.cr_global:sendLocalMeAction(source, "added " .. name .. " to the vehicle.")
				exports['cr_save']:saveVehicleMods(veh)
				exports.cr_logs:dbLog(source, 6, {  veh }, "MODDING ADDUPGRADE " .. name)
			else
	...
	if (veh) then
		...
		if not exports.cr_global:takeMoney(source, mechcost) then
			...
			if setVehicleColor(veh, color1[1], color1[2], color1[3], color2[1], color2[2], color2[3],  color3[1], color3[2], color3[3], color4[1], color4[2], color4[3]) then
				--exports.cr_logs:logMessage("[repaintVehicle] " .. getPlayerName(source)  .. " " ..  getPlayerIP(source)  .. " OR " ..  getPlayerName(client)  .. "/" .. getPlayerIP(client)   .. " changed vehicle " .. getElementData(veh, "dbid") .. " colors to " ..  col1 .. "-" .. col2, 29)
				exports.cr_global:sendLocalMeAction(source, "repaints the vehicle.")
				exports['cr_save']:saveVehicleMods(veh)
				exports.cr_logs:dbLog(source, 6, {  veh }, "MODDING REPAINT " .. toJSON(col))
			end
		end
	end
end
	...
	if veh and stat then
		if stat == 1 then
			...
			--if leader == 1 then
				...
				if not exports.cr_global:takeMoney(source, mechcost) then
					...
					exports.cr_global:sendLocalMeAction(source, "begins to place tint on the windows.")
					...
					if query then
						...
						exports.cr_global:sendLocalMeAction(source, "adds tint to the windows.")
						exports.cr_logs:dbLog(source, 6, {  veh }, "MODDING ADDTINT")
						exports.cr_logs:logMessage("[ADD TINT-LST&R] " .. getPlayerName(source):gsub("_"," ") .. " has added tint to vehicle #" .. vehID .. " - " .. exports.cr_global:getVehicleName(veh) .. ".", 9)
					else
		...
		elseif stat == 2 then
			...
			if not exports.cr_global:takeMoney(source, mechcost) then
				...
				local vehID = getElementData(veh, "dbid")
				exports.cr_global:sendLocalMeAction(source, "begins to remove tint from the windows.")
				...
				if query then
					...
					triggerClientEvent("tintWindows", veh)
					setElementData(veh, "tinted", false, true)
					outputChatBox("You have cleared the tint from the vehicle windows.", source)
					exports.cr_global:sendLocalMeAction(source, "removed tint from the windows.")
					exports.cr_logs:dbLog(source, 6, {  veh }, "MODDING REMOVETINT")
					exports.cr_logs:logMessage("[REMOVED TINT-BTR] " .. getPlayerName(source):gsub("_"," ") .. " has removed tint from vehicle #" .. vehID .. " - " .. exports.cr_global:getVehicleName(veh) .. ".", 9)
				else
