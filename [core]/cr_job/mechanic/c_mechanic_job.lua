...
function paintColor(colorIndex)
	if exports.cr_colorblender:isPickerOpened("paintjob") then
	...
	exports.cr_colorblender:openPicker("paintjob", string.format("#%02X%02X%02X", r, g, b) , "Pick a color for your vehicle:")
end
	...
	if not (wPaint) then
		...
		addEventHandler("onClientGUIClick", bPaintSubmit,  function(button, state)
			if(button == "left" and state == "up") then
				...
				exports.cr_colorblender:closePicker("paintjob")
				closeMechanicWindow()
			end
		end, false)
...
function replaceLightsWindow()
	...
	exports.cr_colorblender:openPicker("headlights", string.format("#%02X%02X%02X", r, g, b) , "Pick a color for the lights:")
end
...
function closeMechanicWindow()
	if exports.cr_colorblender:closePicker("headlights") then
		triggerServerEvent("headlightEndPreview", localPlayer, currentVehicle)
	end
