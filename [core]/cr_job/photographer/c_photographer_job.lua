			...
			if (factionType==6) then
				if (weapon == 43) then
					...
					if(pictureValue==0)then
						...
						outputChatBox("#FF9933Nice, try to snap some more! Picture value: $" .. exports.cr_global:formatMoney(pictureValue), 255, 104, 91, true)
						triggerServerEvent("updateCollectionValue", localPlayer, collectionValue)
					end
					outputChatBox("#FF9933Collection value: $" .. exports.cr_global:formatMoney(collectionValue), 255, 104, 91, true)
				end
			end
		end
	end
...
addEventHandler("updateCollectionValue", localPlayer,
	function(value)
		...
		if value > 0 then
		--	outputChatBox("You still have photos worth $" .. exports.cr_global:formatMoney(collectionValue) .. ".", 255, 194, 14)
		end
	end
)
