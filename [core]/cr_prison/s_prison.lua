...
function startGUI(player)
	...
	if (logged==1) then
		...
		outputChatBox("[!]#FFFFFF Bu komutu yalnızca Los Santos Police Department üyeleri kullanabilir.", player, 255, 0, 0, true)
		end
	end
end
	...
	function(row, removeid, fromGUI)
			...
			if result then
				if not fromGUI then
					...
					exports.cr_logs:dbLog(client, 35, charID, "Removed from jail CharacterID= " .. tostring(t[row][2]))
				else
                ...
                outputChatBox("Error, PS#01. Report on Mantis: bugs.crownroleplay.net", client, 255, 0, 0)
			end
		end
)
	...
	function(name, cell, days, hours, charges, fine, online)
		...
		if query then
			...
			if row then
				...
				local findAccountID = exports.cr_mysql:query("SELECT `id` FROM `mdc_users` WHERE `user` = '" .. playerName .. "'")
				...
				exports.cr_logs:dbLog(client, 35, returnWhat(name, online), "Added to jail cell: " .. row.cell .. " character: " .. row.charactername .. " JailStamp: " .. row.jail_time .. " Charges: " .. row.charges .. " Fine: " .. row.fine)
				if online then
					...
					if tonumber(fine)>0 then -- Issue Fine
						...
						setElementData(name, "bankmoney", bankmoney-amount) -- Take from player
						...
						local tax = exports.cr_global:getTaxAmount() -- Split between gov and PD
						local theTeam = getPlayerTeam(client) 
						exports.cr_global:giveMoney(getTeamFromName("Vice City Police Department"), amount*tax)
					...
					if restrainedObj then
						...
						setElementData(name, "restrain", 0, true)
						setElementData(name, "restrainedBy", false, true)
						setElementData(name, "restrainedObj", false, true)
				...
				outputChatBox("Error, PS#02. Report on Mantis: bugs.crownroleplay.net", client, 255, 0, 0)
			end
		else
	...
	function(name, cell, days, hours, charges, row1, online)
		...
		if query then
			...
			if row then
				...
				table.insert(t, #t+1, { row.id, row.charid, row.charactername, row.jail_time, row.convictionDate, row.updatedBy, row.charges, row.cell, row.fine })
				triggerClientEvent(client, "PrisonGUI:Refresh", client, t)
				exports.cr_logs:dbLog(client, 35, returnWhat(name, online), "Updated prisoner data. New data: " .. row.cell .. " character: " .. row.charactername .. " JailStamp: " .. row.jail_time .. " Charges: " .. row.charges .. " Fine: " .. row.fine)
...
function assignSkin(source)
	...
	-- I put all the id's in g_prison for you.
		...
		local items = exports['cr_items']:getItems(source) -- [] [1] = itemID [2] = itemValue
	...
	if skin then
		...
		mysql:query_free("UPDATE characters SET skin = '" .. exports.cr_mysql:escape_string(skin) .. "', clothingid = '" .. exports.cr_mysql:escape_string(skinID or 0) .. "' WHERE id = '" .. exports.cr_mysql:escape_string(getElementData(source, "dbid")) .. "'")
	end
end
...
function sendPrisonMsg(string)
    ...
    for _, v in ipairs(exports.cr_pool:getPoolElementsByType("player")) do
        local team = getPlayerTeam(v)
        if getTeamFromName("Los Santos County Sheriff Department") == team or getTeamFromName("Los Santos Police Department") == team then
...
function processSpeakerMessage(thePlayer, commandName, ...)
	if not (...) then
		outputChatBox("KULLANIM: /" .. commandName .. " [message] - Sends a speaker-like message through the prison.", thePlayer)
		outputChatBox("You must be in the prison or in the courtyard. SASD usage only.", thePlayer)
	else
		local px, py, pz = getElementPosition(thePlayer)
		if (getElementInterior(thePlayer) == speakerInt and speakerDimensions[getElementDimension(thePlayer)] and exports.cr_faction:isPlayerInFaction(thePlayer, 59)) or (getDistanceBetweenPoints3D(px, py, pz, speakerOutX, speakerOutY, speakerOutZ) < 100 and exports.cr_faction:isPlayerInFaction(thePlayer, 59)) then
			for k, v in ipairs(exports.cr_pool:getPoolElementsByType("player")) do
				...
				if arrayInt == speakerInt and speakerDimensions[arrayDim] then -- speakerInt and speakerDimensions are set in g_ config file
					if exports.cr_faction:isPlayerInFaction(v, 59) then
