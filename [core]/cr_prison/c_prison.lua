...
function PrisonGUIF(result)
        ...
        PrisonGUI.window[1] = guiCreateWindow(x, y, w, h, "Crown Roleplay - Hapishane Arayüzü", false)
        ...
        PrisonGUI.column[1] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Karakter ID", 0.1)
        PrisonGUI.column[7] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Hücre", 0.1)
        PrisonGUI.column[2] = guiGridListAddColumn(PrisonGUI.gridlist[1], "İsim", 0.2)
        PrisonGUI.column[3] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Tarih", 0.15)
        PrisonGUI.column[4] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Gün", 0.1)
        PrisonGUI.column[8] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Saat", 0.1)
        PrisonGUI.column[5] = guiGridListAddColumn(PrisonGUI.gridlist[1], "En son güncelleyen", 0.1)
        PrisonGUI.column[9] = guiGridListAddColumn(PrisonGUI.gridlist[1], "İnce", 0.1)
        PrisonGUI.column[6] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Ücret", 0.5)
        ...
        guiSetProperty(PrisonGUI.button[1], "NormalTextColour", "FFAAAAAA")
         guiSetFont(PrisonGUI.button[1], "default-bold-small")
        ...
        guiSetProperty(PrisonGUI.button[2], "NormalTextColour", "FFAAAAAA")
         guiSetFont(PrisonGUI.button[2], "default-bold-small")
        PrisonGUI.button[3] = guiCreateButton(130, 280, 119, 30, "Suçlu ekle", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[3], "NormalTextColour", "FFAAAAAA")
        guiSetFont(PrisonGUI.button[3], "default-bold-small")
        PrisonGUI.button[4] = guiCreateButton(263, 243, 119, 30, "Suçlu güncelle", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[4], "NormalTextColour", "FFAAAAAA")
        guiSetFont(PrisonGUI.button[4], "default-bold-small")
        PrisonGUI.button[5] = guiCreateButton(588, 242, 134, 30, "Kapıyı aç/kapat", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[5], "NormalTextColour", "FFAAAAAA")
        guiSetFont(PrisonGUI.button[5], "default-bold-small")
        PrisonGUI.button[6] = guiCreateButton(588, 282, 134, 30, "Tüm kapıları aç/kapat", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[6], "NormalTextColour", "FFAAAAAA")   
        guiSetFont(PrisonGUI.button[6], "default-bold-small") 

        --[[
        if pd_offline_jail or exports.cr_integration:isPlayerTrialAdmin(getLocalPlayer()) then
            PrisonGUI.button[7] = guiCreateButton(263, 280, 119, 30, "Çevrimdışı oyuncu ekle", false, PrisonGUI.window[1])
            guiSetProperty(PrisonGUI.button[7], "NormalTextColour", "FFAAAAAA")
            guiSetFont(PrisonGUI.button[7], "default-bold-small") 
...
function addPrisonerGUI(source, name, cell, days, hours, charges, row, fines)
    if isElement(newPrisonerGUI.window[1]) then
        ...
        newPrisonerGUI.window[1] = guiCreateWindow(x, y, w, h, "Yeni suçlu ekle", false)
        ...
        newPrisonerGUI.button[2] = guiCreateButton(170, 241, 123, 28, "Suçlu ekle", false, newPrisonerGUI.window[1])
        ...
        if source==PrisonGUI.button[3] then
            newPrisonerGUI.label[1] = guiCreateLabel(13, 22, 67, 15, "İsim/ID:", false, newPrisonerGUI.window[1])
        ...
        elseif source==PrisonGUI.button[7] then
            newPrisonerGUI.label[1] = guiCreateLabel(15, 22, 67, 15, "Tam İsim:", false, newPrisonerGUI.window[1])
            guiSetFont(newPrisonerGUI.label[1], "default-bold-small")
        ...
        newPrisonerGUI.label[6] = guiCreateLabel(7, 77, 301, 15, "============Mahkumiyet Zamanı==========", false, newPrisonerGUI.window[1])
        ...
        if name then
            ...
            guiSetText(newPrisonerGUI.window[1], "Mahkum Güncelle") -- All about the cosmedics baby
            guiSetText(newPrisonerGUI.button[2], "Mahkum Güncelle")
        ...
        addEventHandler("onClientGUIClick", newPrisonerGUI.button[2], function()
            if overLimit(guiGetText(newPrisonerGUI.edit[3]), guiGetText(newPrisonerGUI.edit[4])) then -- Returns true if over hour limit
                outputChatBox("You are attempting to jail someone over the " .. tonumber(hourLimit) .. " hour limit.", 255, 0, 0) 
                return 
            end
            
            local item = guiComboBoxGetSelected(newPrisonerGUI.edit[2])
            if item == -1 then outputChatBox("Make a cell selection first.", 255, 0, 0) return end
                ...
                if source == PrisonGUI.button[3] then -- Online add
                    if tonumber(guiGetText(newPrisonerGUI.edit[5])) <= 100000 then
                        if tonumber(guiGetText(newPrisonerGUI.edit[4]))+tonumber(guiGetText(newPrisonerGUI.edit[3])) >= 1 then
                            local textLength = string.len(guiGetText(newPrisonerGUI.memo[1]))
                            if textLength < 10 then return outputChatBox("Geçerli bir sebep girmeniz gerekiyor.", 255, 0, 0)  end
                            triggerServerEvent("addPrisoner", resourceRoot, user, cell, guiGetText(newPrisonerGUI.edit[3]), guiGetText(newPrisonerGUI.edit[4]), guiGetText(newPrisonerGUI.memo[1]), math.floor(guiGetText(newPrisonerGUI.edit[5])), true)
                        else
...
function updateGates()
    ...
    PrisonGUI.label[1] = guiCreateLabel(750, 287, 100, 30, open .. " adet kapı kapalı", false, PrisonGUI.window[1])
    PrisonGUI.label[2] = guiCreateLabel(750, 247, 100, 30, closed .. " adet kapı açık", false, PrisonGUI.window[1])
    guiLabelSetColor(PrisonGUI.label[1], 0, 255, 0)
    guiLabelSetColor(PrisonGUI.label[2], 255, 0, 0)
    guiSetFont(PrisonGUI.label[2], "default-bold-small") 
    guiSetFont(PrisonGUI.label[1], "default-bold-small") 
end

function prisonPed()
    local ped = createPed(283, 214.21875, 120.646484375, 999.015625)
    setPedAnimation(ped, "COP_AMBIENT", "Coplook_loop", -1, true, false, false)
    setElementFrozen(ped, true)
    setElementRotation(ped, 0, 0, 270.56)
