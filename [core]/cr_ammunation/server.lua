		...
		if getElementData(colShape, "mermiYeri") then
			if isElementWithinColShape(thePlayer, colShape) then
				...
				if not playerTeam then
					outputChatBox("[!]#FFFFFF Herhangi bir birliğiniz yok!", thePlayer, 255, 0, 0, true)
					return false
				end
				if playerTeamType == 1 or playerTeamType == 0 then
					...
					outputChatBox("[!]#FFFFFF Bu işlemi yapabilmek için bir illegal birliğe dahil olmanız gerekmektedir!", thePlayer, 255, 0, 0, true)
				end
			end
		end
	end
			...
			if isElementWithinColShape(thePlayer, colShape) then
				...
				if playerTeamType == 1 or playerTeamType == 0 then
					if not silahTuru or not silahTurleri[string.lower(silahTuru)] or not sarjorMiktari then
						outputChatBox("[!]#FFFFFF KULLANIM: /" .. cmd .. " [Mermi Türü] [Şarjör Miktarı]", thePlayer, 255, 0, 0, true)
						return false
					end
					if silahTurleri[string.lower(silahTuru)] then
						local playerWeapons = getPedWeapons(thePlayer)
						if not playerWeapons[1] then
							outputChatBox("[!]#FFFFFF Bu mermiyi satın almak için uygun silahınız yok.", thePlayer, 255, 0, 0, true)
							return false
						end
						...
						for index, weaponID in ipairs(playerWeapons) do
							if weaponID == silahTurleri[string.lower(silahTuru)][1] then
								...
								outputChatBox("[!]#FFFFFF Toplam tutar: " .. toplamFiyat .. "$ Satışı onaylamak için /mermionayla, iptal etmek için /mermiiptal yazın!", thePlayer, 0, 0, 255, true)
...
function mermiOnayla(thePlayer, cmd)
	...
	if mermiTutari then
		if exports.cr_global:hasMoney(thePlayer, mermiTutari) then
			...
			exports.cr_global:takeMoney(thePlayer, mermiTutari)
			for i = 1, sarjorMiktari do
				give, error = exports.cr_global:giveItem(thePlayer, 116, silahTuru..":"..mermiMiktari..":Silah Mermisi "..getWeaponNameFromID(silahTuru))
			end
			...
			outputChatBox("[!]#FFFFFF Satış başarıyla gerçekleştirildi! " .. sarjorMiktari .. " Şarjör (" .. mermiMiktari .. ") için toplam " .. mermiTutari .. "$ ödediniz!", thePlayer, 0, 255, 0, true)
		else
...
function mermiIptal(thePlayer, cmd)
	...
	if mermiTutari then
		...
		setElementData(thePlayer, "mermi:sarjorMiktari", false)
		setElementData(thePlayer, "mermi:silahTuru", false)
		outputChatBox("[!]#FFFFFF Satışı başarıyla iptal ettiniz!", thePlayer, 0, 255, 0, true)
	end
end
