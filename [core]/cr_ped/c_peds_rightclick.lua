...
function clickPed(button, state, absX, absY, wx, wy, wz, element)
	...
	if (element) and (getElementType(element)=="ped") and (button=="right") and (state=="down") and (sent==false) and (element~=getLocalPlayer()) then
		...
		if (interact and interact ~= "false") then
			...
			if (getDistanceBetweenPoints3D(x, y, z, wx, wy, wz)<=3) then
				...
				if(interact == "ch.reception") then
					rcMenu = exports.cr_rightclick:create(name)
					row.talk = exports.cr_rightclick:addrow("Konuş")
				...
				elseif(interact == "election") then --Election ped
					rcMenu = exports.cr_rightclick:create(name)
					row.talk = exports.cr_rightclick:addrow("Vote")
					addEventHandler("onClientGUIClick", row.talk,  function (button, state)
				...
				elseif(interact == "mission.clarice") then
					...
					row.util = exports.cr_rightclick:addrow("WooHoo_clarice")
				...
				elseif(interact == "santa") then
					rcMenu = exports.cr_rightclick:create(name)
					row.get = exports.cr_rightclick:addrow("Get Coke")
					...
					if exports.cr_global:hasItem(getLocalPlayer(), 211) then --has christmas lottery ticket
						row.claim = exports.cr_rightclick:addrow("Claim Prize")
				...
				elseif(interact == "pilotmission") then
					rcMenu = exports.cr_rightclick:create(name)
					row.getlisting = exports.cr_rightclick:addrow("Get listing")
					addEventHandler("onClientGUIClick", row.getlisting, function ()
							triggerEvent("pilotmission:startGUI", getResourceRootElement(getResourceFromName("cr_sfia")))
						end, false)
					local temp_table = getElementData(getResourceRootElement(getResourceFromName("cr_sfia")), "sfia_pilots:table")
					local name = string.gsub(getPlayerName(getLocalPlayer()), "_", " ")
					for k, v in pairs(temp_table) do
						if v["charactername"] == name then
							row.gettask = exports.cr_rightclick:addrow("Get task")
							addEventHandler("onClientGUIClick", row.gettask, function ()
									triggerEvent("pilotmission:domission", getResourceRootElement(getResourceFromName("cr_sfia")))
								end, false)
							break
						end
					end
				...
				elseif(interact == "k9") then
					rcMenu = exports.cr_rightclick:create(getElementData(element, "rpp.npc.name") or "Dog")
					if getElementData(element, "besitzer") == getPlayerName(getLocalPlayer()) then
						row.talk = exports.cr_rightclick:addrow("Status")
						addEventHandler("onClientGUIClick", row.talk,  function (button, state)
				...
				elseif(interact == "cargo.generics") then
					if getElementData(localPlayer, "loggedin") == 1 then
		                ...
		                for k,v in ipairs(allowedFactions) do
		                    local isIn, _, leader = exports.cr_faction:isPlayerInFaction(localPlayer, v)
				...
				elseif(interact == "cargo.shippingorder") then
					rcMenu = exports.cr_rightclick:create(name)
					row.talk = exports.cr_rightclick:addrow("Shipping orders")
					addEventHandler("onClientGUIClick", row.talk,  function (button, state)
				...
				if stretcherElement then
					...
					if stretcherPlayer and stretcherPlayer == player then
						...
						row.stretcher = exports.cr_rightclick:addrow("Take from stretcher")
					...
					if not stretcherPlayer then
						...
						row.stretcher = exports.cr_rightclick:addrow("Lay on stretcher")
						addEventHandler("onClientGUIClick", row.stretcher, function (button, state)
				...
				if(getElementHealth(element) < 50) then
					if exports.cr_faction:isInFactionType(getLocalPlayer(), 4) then --if player is in ES faction and has a first aid kit
						if not rcMenu then
							rcMenu = exports.cr_rightclick:create(name)
						end
						row.heal = exports.cr_rightclick:addrow("Heal")
		...
		if (getElementData(element, "rpp.npc.dbid")) then
			if (exports.cr_integration:isPlayerGameAdmin(localPlayer) and exports.cr_global:isStaffOnDuty(localPlayer)) or exports.cr_integration:isPlayerScripter(localPlayer) then
				...
				--if(getElementData(element, "rpp.npc.spawnpos")) then
					...
					--if getElementData(element, "dbid") then
						row.admEdit = exports.cr_rightclick:addrow("ADM: Edit")
						addEventHandler("onClientGUIClick", row.admEdit, function (button, state)
							adminEditPedGui(element)
						end, false)
					--end

					row.respawn = exports.cr_rightclick:addrow("ADM: Respawn")
			...
			--[[if (exports.cr_integration:isPlayerHeadAdmin(localPlayer) and exports.cr_global:isStaffOnDuty(localPlayer)) or exports.cr_integration:isPlayerScripter(localPlayer) then
				local name = getElementData(element, "rpp.npc.name")
				if not rcMenu then
					rcMenu = exports.cr_rightclick:create(name or "NPC")
				end
				row.delete = exports.cr_rightclick:addrow("ADM: Temporary delete ped")
