...
function loadAllPeds(res)
	...
	connection, "SELECT id FROM `peds` ORDER BY `id` ASC")
end
...
function reloadPed(id)
	...
	local thePed = exports.cr_pool:getElement("ped", tonumber(id))
...
function loadOnePed(id)
	...
	local row = mysql:query_fetch_assoc("SELECT * FROM peds WHERE id = " .. (id) .. " LIMIT 1")
	if row then
		...
		if ped then
			setElementData(ped, "dbid", row.id)
			setElementData(ped, "rpp.npc.dbid", row.id)
			exports.cr_pool:allocateElement(ped, row.id, true)
			...
			--setElementData(ped, "rpp.npc.spawnpos", row.x .. "," .. row.y .. "," .. row.z .. "," .. row.rotation .. "," .. row.interior .. "," .. row.dimension)
			...
			if row.type then
				setElementData(ped, "rpp.npc.type", row.type)
			end
			setElementData(ped, "rpp.npc.name", pedname)
			setElementData(ped, "ped:name", pedname) -- For chat system
			setElementData(ped, "rpp.npc.gender", gender)
			setElementData(ped, "name", pedname) --for owl
			if row.nametag then
				setElementData(ped, "rpp.npc.nametag", true)
				setElementData(ped, "nametag", true) --for owl
			end
			setElementData(ped, "rpp.npc.behav", row.behaviour)
			if row.money then --if ped have money, set the amount
				setElementData(ped, "rpp.npc.money", row.money)
				...
				if(row.owner_type ~= 0 and row.owner ~= 0) then
					setElementData(ped, "rpp.npc.owner_type", row.owner_type)
					setElementData(ped, "rpp.npc.owner", row.owner)
				end
			end
			...
			if row.animation then
				setElementData(ped, "rpp.npc.animation", row.animation)
				--outputDebugString(tostring(row.animation))
				local animTable = exports.cr_global:split(row.animation, ";")
			...
			local badges = {}
			badges = exports['cr_items']:getBadges()
			--outputDebugString(tostring(#badges) .. " badges")
			for k,v in pairs(badges) do
				local hasItem = exports['cr_items']:hasItem(ped, k)
				if hasItem then
					local itemresult = exports['cr_items']:npcUseItem(ped, k)
					--outputDebugString("ped itemresult = " .. tostring(itemresult))
					break;
				end
			end
			local masks = {}
			masks = exports['cr_items']:getMasks()
			...
			--exports['cr_items']:updateLocalGuns(ped)
			
			setElementData(ped, "rpp.npc.createdBy", row.created_by)
			setElementData(ped, "rpp.npc.createdByUsername", exports.cr_global:getUserNameFromID(row.created_by))
			setElementData(ped, "rpp.npc.createdAt", row.created_at)
			setElementData(ped, "rpp.npc.comment", row.comment)
			setElementData(ped, "rpp.npc.synced", tonumber(row.synced) == 1 or false)
			
			--DEBUG / TESTING ONLY:
			--[[
			local itemID = 65
			
			local itemresult = exports['cr_items']:npcUseItem(ped, itemID)
			outputDebugString("ped itemresult = " .. tostring(itemresult))
			--]]
		end
	end
...
--[[
function createPeds()
	--local nametags = getResourceState(getResourceFromName("cr_nametags"))
	...
	--	exports.cr_nametags:addNametag(peds[1])
	--end
	...
	--if(nametags == "running") then
	--	exports.cr_nametags:addNametag(peds[2])
	--end
end
...
function healByHospitalPed(ped)
	...
	if(needed < 5) then
		exports.cr_RPPchat:chatPed(ped, "say", "You seem to be fine, sir. No treatment needed.")
	else
		...
		if(getPlayerMoney(source) > cost or getPlayerMoney(source) == cost) then
			...
			exports.cr_RPPchat:chatMe(source, false, "takes out a wallet and hands a few dollar notes to " .. pedname)
			exports.cr_RPPchat:chatPed(ped, "me", "takes the money")
			takePlayerMoney(source, tonumber(cost))
			exports.cr_RPPchat:chatPed(ped, "say", "Thank you sir. I'll get you fixed up.")
			exports.cr_RPPchat:chatPed(ped, "me", "heals " .. playername)
			setElementHealth(source, newHealth)
		else
			exports.cr_RPPchat:chatMe(source, false, "takes out a wallet and hands a few dollar notes to " .. pedname)
			exports.cr_RPPchat:chatPed(ped, "say", "Sorry sir. I don't think that is enough.")
		end
	end
end
...
function makeTempPed(thePlayer, command, behav, gender, skin, name) --create temporary ped (no dbid)
	if exports.cr_integration:isPlayerTrialAdmin(thePlayer) then
		...
		if name then
			local splitname = exports.cr_global:split(name, "_")
			if(#splitname >= 2) then
...
function makeFuelPed(thePlayer, command) --Create temporary fuel ped (no dbid)
	if global:isPlayerGameAdmin(thePlayer) then
		local x,y,z = getElementPosition(thePlayer)
		local rx,ry,rz = getElementRotation(thePlayer)
		local int = getElementInterior(thePlayer)
		local dim = getElementDimension(thePlayer)
		theNewPed = createPed (50, x, y, z)
		exports.cr_pool:allocateElement(theNewPed)
		setPedRotation (theNewPed, rz)
		setElementFrozen(theNewPed, true)
		--setPedAnimation(theNewPed, "FOOD", "FF_Sit_Loop",  -1, true, false, true)
		setElementData(theNewPed, "rpp.npc.type", "fuel", true)
		setElementData(theNewPed, "rpp.npc.name", getRandomName("full", "male"), true)
		setElementData(theNewPed, "rpp.npc.nametag", "true", true)
		setElementData(theNewPed, "rpp.npc.behav", "1", true)

		setElementData(theNewPed, "fuel:priceratio" , 100, false)

		-- For the language system
		setElementData(theNewPed, "languages.lang1" , 1, false)
		setElementData(theNewPed, "languages.lang1skill", 100, false)
		setElementData(theNewPed, "languages.lang2" , 2, false)
		setElementData(theNewPed, "languages.lang2skill", 100, false)
		setElementData(theNewPed, "languages.current", 1, false)
	end
end
...
function pedMoveToTram(ped, tram)
	...
	--else
		...
		pedMoveTimer[ped] = nil
		setPedAnimation(ped)
		exports.cr_trams:npcEnterTramPassenger(ped, tram)
	--end
end
		...
		if(getElementType(hitElement) == "vehicle") then
			if(getElementModel(hitElement) == 449) then
				...
				if(getVehicleController(hitElement)) then
					if(getResourceState(getResourceFromName("cr_trams")) == "running") then
...
function respawnPed(player, ped)
	if(getElementType(ped) == "ped") then
		if getElementData(ped, "dbid") then
			...
			--if(ped == exports.cr_pool:getElement("ped", tonumber(dbid))) then --extra security check
			...
			--else
			--	outputDebugString("Failed to respawn ped with dbid " .. tostring(dbid) .. " (#1) (" .. tostring(ped) .. " = " .. tostring(exports.cr_pool:getElement("ped", tonumber(dbid))) .. ")")
			--	return false
			--end
		...
		elseif getElementData(ped, "rpp.npc.spawnpos2") then
			...
			local pos = exports.cr_global:split(posstring, ",")
...
function healPed(player, ped)
	if(getElementType(ped) == "ped") then
		...
		if not (factionType==4) then
			...
			local foundkit, slot, itemValue = exports.cr_global:hasItem(player, 70)
			if foundkit then
				if isPedDead(ped) then
					...
					if itemValue > 1 then
						exports['cr_items']:updateItemValue(player, slot, itemValue - 1)
					else
						exports.cr_global:takeItem(thePlayer, 70, itemValue)
						if not exports.cr_global:hasItem(thePlayer, 70) then
					...
					local price = 100									
					exports.cr_global:giveMoney(getTeamFromName("San Fierro Emergency Services"), math.ceil((1-tax)*price))
					exports.cr_global:giveMoney(getTeamFromName("Government of San Fierro"), math.ceil(tax*price))
					if pedName then
						outputChatBox("You healed " ..pedName.. " (NPC).", player, 0, 255, 0)
					else
						outputChatBox("You healed a NPC.", player, 0, 255, 0)
					end
					exports.cr_logs:dbLog(player, 35, ped, "HEAL NPC FOR $" .. price)
				end
			else
...
function deletePed(player, ped)
	if(getElementType(ped) == "ped") then
		if exports.cr_integration:isPlayerGameAdmin(player) then
			local dbid = tonumber(getElementData(ped, "rpp.npc.dbid")) or 0
			if dbid > 0 then
				local pedFunction = getElementData(ped, "rpp.npc.type")
				if pedFunction then
					--if(allowedFunctionPeds[pedFunction] or exports.cr_global:isPlayerManagement(player)) then
				...
				destroyElement(ped)
				exports.cr_global:sendMessageToAdmins("[Peds] Temp NPC '" .. tostring(pedName) .. "' was deleted by " .. getPlayerName(player) .. ".")
			end
		end
	end
end
...
function deleteDbPed(player, pedID, ped)
	...
	if exports.cr_integration:isPlayerTrialAdmin(player) then
		if not ped then
			ped = exports.cr_global:getPoolElement("ped", pedID)
		end
		if not ped then
		...
		if pedType then
			if(not allowedFunctionPeds[pedFunction]) then
				if not exports.cr_integration:isPlayerGameAdmin(player) then
