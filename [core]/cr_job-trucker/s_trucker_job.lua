...
--  Model   (1)Capacity (2)Level (3)CrateWeight, (4)Kgs
...
function spawnRoute(thePlayer, spawnNewRoute)
	...
	if currentSpot and not spawnNewRoute then
		...
		if not selectedRoute then
			--exports.cr_hud:sendBottomNotification(thePlayer, "RS Haul Operator:", "There is no customer orders to do at the moment, please stand by...")
		else
			setElementData(thePlayer, "job-system-trucker:currentRoute", selectedRoute)
...
function selectAFreeSpot(thePlayer)
	...
	if not routes[randIndex] then
		selectSpotTries[getElementData(thePlayer, "dbid")] = (selectSpotTries[getElementData(thePlayer, "dbid")] or 0)+ 1
		if selectSpotTries[getElementData(thePlayer, "dbid")] > 20 then
			selectSpotTries[getElementData(thePlayer, "dbid")] = 0
			return false
		else
			return selectAFreeSpot(thePlayer)
		end
	end

	if routes[randIndex][5] and isElement(routes[randIndex][5]) and getElementType(routes[randIndex][5]) == "player" then
		selectSpotTries[getElementData(thePlayer, "dbid")] = (selectSpotTries[getElementData(thePlayer, "dbid")] or 0)+ 1
		if selectSpotTries[getElementData(thePlayer, "dbid")] > 20 then
			selectSpotTries[getElementData(thePlayer, "dbid")] = 0
			return false
	...
	routes[randIndex].index = randIndex
	selectSpotTries[getElementData(thePlayer, "dbid")] = 0
...
function freeSpot(thePlayer)
	...
	if cur then
		...
		setElementData(thePlayer, "job-system-trucker:currentRoute", nil) 
	end
end
...
function cleanUp(reason)
	if getElementData(source, "job") == 1 then
		...
		if truckingRuns and truckingRuns ~= 0 then
			dbExec(mysql:getConnection(),"UPDATE `jobs` SET `jobTruckingRuns`='" .. tostring(truckingRuns) .. "' WHERE `jobCharID`='" .. tostring(getElementData(source, "dbid")) .. "' AND `jobID`='1' ")
		end
	end
end
...
function giveTruckingMoney(vehicle)
	...
	if takenWeight then
		--exports.cr_hud:sendBottomNotification(source, "RS Haul Operator:", "RS Haul has unloaded " .. takenWeight .. " Kg(s) of supplies remaining in the back. ")
	end
	
	-- level up and reset runs/wage
	local vehicle = getPedOccupiedVehicle(source)
	if getElementData(vehicle, "job") ~= 1 then
		--exports.cr_hud:sendBottomNotification(source, "RS Haul Operator:", "Man..You have to use RS Haul vehicle.")
	else
		local truckModel = getElementModel(vehicle)
		local truck = truckerJobVehicleInfo[truckModel]
		if truck then
			...
			if truckruns > 0 then
				if truckrunsTilNextLevel then
					...
					if truckrunCarry >= 0 then -- level up
						...
						dbExec(mysql:getConnection(),"UPDATE `jobs` SET `jobLevel`='" .. tostring(currentJobLevel+1) .. "', `jobProgress`='" .. (tostring(truckrunCarry)) .. "', `jobTruckingRuns`='0' WHERE `jobID`='1' AND `jobCharID` = '" ..tostring(charID) .. "' ")
						...
						dbExec(mysql:getConnection(),"UPDATE `jobs` SET `jobProgress`='" .. tostring(currentProgress+truckruns) .. "', `jobTruckingRuns`='0' WHERE `jobID`='1' AND `jobCharID` = '" ..tostring(charID) .. "' ")
					end
				else
					dbExec(mysql:getConnection(),"UPDATE `jobs` SET `jobProgress`='" .. tostring(currentProgress+truckruns) .. "', `jobTruckingRuns`='0' WHERE `jobID`='1' AND `jobCharID` = '" ..tostring(charID) .. "' ")
				end
				
				exports["cr_job"]:fetchJobInfoForOnePlayer(source)
				
				if not notified then
					if not truckrunsTilNextLevel then
						--exports.cr_hud:sendBottomNotification(source, "Delivery Job New Achievement!", "Progress: " .. (getElementData(source, "jobProgress") or 0) .. " truck runs (You mastered this job).")
					else
						--exports.cr_hud:sendBottomNotification(source, "Delivery Job New Achievement!", "Progress: " .. math.floor((getElementData(source, "jobProgress") or 0)/truckrunsTilNextLevel*100) .. "%")
					end
				end
				playSoundFX(vehicle)
			end
	...
	-- RESET SHIT
	freeSpot(source)
	setElementData(source, "job-system-trucker:currentRoute", false)
	setElementData(source, "job-system-trucker:truckruns", 0)
	setElementData(source, "job-system-trucker:currentRouteID", -1)
...
function takeRemainingCrates(vehicle)
	if vehicle then
		local weight = 0
		for key, item in pairs(exports["cr_items"]:getItems(vehicle)) do 
			if tonumber(item[1]) == 121 then
				if exports.cr_global:takeItem(vehicle, item[1], item[2]) then
	...
	if getElementData(source,"job") == 1 and truckerJobVehicleInfo[truckModel] then
		...
		if playerJobLevel < truckLevelRequire then
			...
			if truckLevelRequire == 1 then
				--exports.cr_hud:sendBottomNotification(thePlayer, "RS Haul Operator:", "You're not RS Haul Employee, please register for this job at City Hall.")
			else
				--exports.cr_hud:sendBottomNotification(thePlayer, "RS Haul Operator:", "You're required Delivery Driver Level " .. truckLevelRequire .. " Certificate to drive this " .. truckName .. ".")
			end
...
function checkTruckingEnterVehicle(thePlayer, seat)
	...
	if seat == 0 and getElementData(source,"job") == 1 and getElementData(thePlayer,"job") == 1 then
		if true then --exports.cr_integration:isPlayerScripter(thePlayer) then
			...
			if curentCrates <= 0 then
				--exports.cr_hud:sendBottomNotification(thePlayer, "RS Haul Operator: Your truck is empty!", "Return to RS Haul station to reload your truck with supplies crates!")
			else
				--exports.cr_hud:sendBottomNotification(thePlayer, "RS Haul Operator: ", "Your vehicle has " .. curentCrates .. " Kg(s) of supplies in the back. Deliver them to our customer's places!")
			...
			--exports.cr_hud:sendBottomNotification(thePlayer, "Farid: ", "We're upgrading trucker job system. Please try another job in the meantime.")
		end
...
function startLoadingUp()
	local vehicle = getPedOccupiedVehicle(source)
	if not vehicle then
		--exports.cr_hud:sendBottomNotification(source, "RS Haul Operator: ", "Man, where is your truck?")
		return false
	end
	if getElementData(vehicle, "job") ~= 1 then
		--exports.cr_hud:sendBottomNotification(source, "RS Haul Operator: ", "Man..You have to use RS Haul vehicle.")
	...
	if truck then
		...
		local crateWeight = math.min(truck[3], truck[4] - curentCrates)

		if crateWeight > 0 and exports["cr_items"]:giveItem(vehicle, 121, crateWeight) then
			playSoundFX(vehicle)
			if curentCrates > 0 and not getElementData(source, "job-system-trucker:currentRoute") then
				spawnRoute(source)
			end
		else
			--exports.cr_hud:sendBottomNotification(source, "RS Haul Operator: ", "Your truck is full and can not load anymore of supplies! Drive to the yellow blips to complete deliveries.")
			triggerClientEvent(source, "job-system:trucker:leaveStationLoadup", source, true)
		end
...
function checkIfPlayerTruckHasEnoughtShit()
	...
	local truck = truckerJobVehicleInfo[truckModel]
	if not truck then
		--exports.cr_hud:sendBottomNotification(source, "RS Haul Operator: ", "Man..RS Haul doesn't allow this model of vehicle.")
		return false
	end
	local success, droppedWeight = unloadCrates(source, vehicle)
	if success then
		local earned = math.ceil((droppedWeight*(math.random(5,10)/5))*(getElementHealth(vehicle)/1000)) or 0
		earned = math.ceil(earned * 0.7)
		local formartedEarned = exports.cr_global:formatMoney(tostring(earned))
		
		exports.cr_global:giveMoney(source, earned)	
		setElementData(source, "job-system-trucker:truckruns",(getElementData(source, "job-system-trucker:truckruns") or 0) + 1)
	
		--exports.cr_hud:sendBottomNotification(source, "RS Haul Operator: ", "Customer paid you $" .. formartedEarned .. " for " .. droppedWeight .. " Kg of supplies you unloaded from the truck.")
		
		playSoundFX(vehicle)
		
	else
		--exports.cr_hud:sendBottomNotification(source, "RS Haul Operator: ", droppedWeight .. " Return to RS Haul's warehouse first.")
	end
	...
	if getVehicleOccupant(veh) == thePlayer then
		...
		if currentSpot then
			...
			if currentSpot[4] then --Weight Required
				...
				setElementData(thePlayer, "job-system-trucker:currentRoute", currentSpot)
			end
			
			local whatPlayerHas = 0
			for key, item in pairs(exports["cr_items"]:getItems(veh)) do 
				if item[1] == 121 then
			...
			if whatPlayerHas > 0 then
				for key, item in pairs(exports["cr_items"]:getItems(veh)) do 
					if item[1] == 121 then
						if tonumber(item[2]) then
							exports["cr_items"]:takeItem(veh, 121,item[2])
						end
					end
				end
				playSoundFX(veh)
			...
			if currentSpot[8] and currentSpot[8] > 0 then --This is actual order from other player
				if carry >= 0 then -- If truck has enough supplies to solved the spot fully
					if carry > 0 then
						exports["cr_items"]:giveItem(veh, 121,tostring(carry))
					end
			...
			return false, "((Script Error Code 1))"
		end
	else
		return false, "You can not use other people's vehicle."
	end
	return false, "((Script Error Code 2))"
end
...
function removeActualRouteAndOrder(orderID, intID, orderWeight, remainingOrder, addToInt)
	if not remainingOrder then --Clear all
		...
		if not dbExec(mysql:getConnection(),"DELETE FROM `jobs_trucker_orders` WHERE `orderID`='" .. orderID .. "' ") then
			if debugmode then
		...
		if not dbExec(mysql:getConnection(),"UPDATE `jobs_trucker_orders` SET `orderWeight`='" .. remainingOrder .. "' WHERE `orderID`='" .. orderID .. "' ") then
			if debugmode then
...
function updateInteriorSupply(intID, supplies)
	...
	if dbExec(mysql:getConnection(),"UPDATE `interiors` SET `supplies`=`supplies`+'" .. tostring(supplies) .. "' WHERE `id`='" .. intID .. "' ") then
		exports["cr_interior"]:realReloadInterior(tonumber(intID))
		if debugmode then
...
function updateNextCheckpoint(pointid)
	...
	setElementData(source, "job-system-trucker:currentRouteID",pointid)
end
...
function cleanup ()
	for key, thePlayer in pairs(getTruckers()) do
		--exports.cr_hud:sendBottomNotification(thePlayer, "Delivery Job Script Update", "A Developer has updated the trucker job system. Please re-enter the truck if you're currently in one.")
		setElementData(thePlayer, "job-system-trucker:currentRoute", false)
	end
	fetchActualOrders()
