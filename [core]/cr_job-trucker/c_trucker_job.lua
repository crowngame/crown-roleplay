...
function displayTruckerJob(notext, spwan)
	-- if (jobstate==0) then
		...
		if not notext then
			--exports.cr_hud:sendBottomNotification(localPlayer, "RS Haul Operator:", "Approach the Grey Truck Icon on your radar and enter the RS Haul's vehicle to start your job.")
		end
	-- end
end
...
function waitAtStationLoadup(thePlayer)
	...
	if thePlayer == localPlayer and vehicle and getVehicleController(vehicle) == localPlayer then
		if getElementHealth(vehicle) < 900 then
			--exports.cr_hud:sendBottomNotification(localPlayer, "RS Haul Operator:", "You need to get your truck repaired first .. ")
		else
			if not timerLoadUp then
				--exports.cr_hud:sendBottomNotification(thePlayer, "RS Haul Operator:", "Now waiting a moment while your truck is being loaded up with supply crates.")
...
function endDelivery(thePlayer)
	if thePlayer == localPlayer then
		...
		if not vehicle or getVehicleController(vehicle) ~= localPlayer then
			--exports.cr_hud:sendBottomNotification(thePlayer, "RS Haul Operator:", "You must be in a RS Haul's vehicle to complete deliverys.")
		else
			local health = getElementHealth(vehicle)
			if health <= 900 then
				--exports.cr_hud:sendBottomNotification(thePlayer, "RS Haul Operator:", "This truck is damaged, fix it first.")
			else
...
function waitAtDelivery(thePlayer)
	...
	if thePlayer == localPlayer and vehicle and getVehicleController(vehicle) == localPlayer then
		if getElementHealth(vehicle) < 900 then
			--exports.cr_hud:sendBottomNotification(thePlayer, "RS Haul Operator:", "You need to get your truck repaired.")
		else
			deliveryStopTimer = setTimer(checkIfPlayerTruckHasEnoughtShit, 5000, 1)
			killTimerCountDown()
			--exports.cr_hud:sendBottomNotification(thePlayer, "RS Haul Operator:", "Wait a moment while your truck is processed.")
			addEventHandler("onClientMarkerLeave", marker, checkWaitAtDelivery)
		end 
	end
end
...
function checkIfPlayerTruckHasEnoughtShit()
	...
	if vehicle and getVehicleController(vehicle) == localPlayer  then
		if getElementData(vehicle, "job") ~= 1 then
			--exports.cr_hud:sendBottomNotification(localPlayer, "RS Haul Operator:", "Man..You have to use RS Haul vehicle.")
			return false
		end
		--spawnFinishMarkerTruckJob()
		triggerServerEvent("truckerjob:checkIfPlayerTruckHasEnoughtShit", localPlayer)	
	else
		--exports.cr_hud:sendBottomNotification(localPlayer, "RS Haul Operator:", "You must be in the Truck to complete deliverys.")
	end
end