...
function addactualorder(thePlayer, commandName, orderWeight, InteriorID, ...)
	if (exports.cr_integration:isPlayerScripter(thePlayer)) then
		if not tonumber(orderWeight) or not tonumber(InteriorID) or not (...) then
			outputChatBox("KULLANIM: /" .. commandName .. " [orderWeight] [InteriorID] [Location Name]", thePlayer, 255, 194, 14)
			return false
		end
		local orderName = table.concat({...}, " ")
		local x, y, z = getElementPosition(thePlayer)
		if dbExec(mysql:getConnection(),"INSERT INTO `jobs_trucker_orders` SET `orderX`='" .. tostring(x) .. "', `orderY`='" .. tostring(y) .. "', `orderZ`='" .. tostring(z) .. "', `orderWeight`='" .. tostring(math.floor(tonumber(orderWeight))) .. "', `orderName`='" .. tostring(orderName):gsub("'","''") .. "', `orderInterior`='" .. tostring(math.floor(tonumber(InteriorID))) .. "' ") then
			outputChatBox("Successfully added actual order '" .. orderName .. "' into SQL manuanlly.", thePlayer, 0,255,0)
			fetchActualOrders()
			if debugmode then
				--outputDebugString("Successfully added actual order '" .. orderName .. "' into SQL manuanlly.")
			end
		else
			outputChatBox("Failed to add actual order '" .. orderName .. "' into SQL manuanlly.", thePlayer, 255,0,0)
			if debugmode then
				--outputDebugString("Failed to add actual order '" .. orderName .. "' into SQL manuanlly.")
			end
		end
	else
		outputChatBox("Only Super Admin can access /" .. commandName .. ".", thePlayer, 255,0,0)
	end
end
...
function addOrderManually(thePlayer, commandName , ...)
	if (exports.cr_integration:isPlayerScripter(thePlayer)) then
		if not (...) then
			outputChatBox("KULLANIM: /" .. commandName .. " [Location Name]", thePlayer, 255, 194, 14)
			return false
		end
		local orderName = table.concat({...}, " ")
		local x, y, z = getElementPosition(thePlayer)
		if dbExec(mysql:getConnection(),"INSERT INTO `jobs_trucker_orders` SET `orderX`='" .. tostring(x) .. "', `orderY`='" .. tostring(y) .. "', `orderZ`='" .. tostring(z) .. "', `orderName`='" .. tostring(orderName):gsub("'","''") .. "' ") then
...
function delMarker(id)
	if dbExec(mysql:getConnection(),"DELETE FROM `jobs_trucker_orders` WHERE `orderID`='" .. tostring(id) .. "' ") then
		local deleted = false
		for i = 1, #routes do
		...
		if deleted then
			...
			outputChatBox("Faled to delete marker ID #" .. id .. ".", client, 255,0,0)
		end
	end
end
