...
function ramp_add (p, cmd)
    ...
    local ty = y + radius * math.cos (math.rad (rz))
		...
		local query = mysql:query_free ("INSERT INTO ramps SET id=" .. id .. ",position='" .. position .. "',interior='" .. interior .. "',dimension='" .. dimension .. "',rotation=" .. math.ceil (rz) .. ",creator='" .. creator .. "', state=0")
		
		if query then
			ramp_load (id)
			outputChatBox ("Created Ramp with ID " .. id  .. ".", p, 0, 255, 0, false)
			exports.cr_global:giveItem (p, 151, id)
			exports.cr_global:sendMessageToAdmins ("AdmWarn: " .. creator .. " created a ramp with ID " .. id)
		else
...
function ramp_delete (p, cmd, id)
    ...
    local query = mysql:query_free ("DELETE FROM ramps WHERE id = " .. id)
		...
		if query then
			...
			exports.cr_global:sendMessageToAdmins ("AdmWarn: " .. deleter .. " deleted a ramp with ID " .. id)
		else
...
--[[
local good = aclReload()
if good thenm
    outputChatBox ("ACL successfully reloaded", root, 0, 255, 0, false)
else
    outputChatBox ("Eek, failed to reload ACL", root, 255, 0, 0, false)
end
]]