...
function ramp_init ()
    ...
    if result then
       while true do
        ...
        exports.cr_global:sendMessageToAdmins ("AdmWarn: Failed to select ramps from MySQL Database, please panic.")
    end
...
function gotoRamp (p, commandName, target)
    if exports.cr_integration:isPlayerTrialAdmin(p) then
	if not target then
		outputChatBox("KULLANIM: /" .. commandName .. " [Ramp ID]", p, 255, 194, 14)
		else
		for i,v in ipairs (getElementsByType ("object")) do
