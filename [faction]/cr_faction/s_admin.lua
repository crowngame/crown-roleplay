...
function loadWelfare()
	local result = exports.cr_mysql:query_fetch_assoc("SELECT value FROM settings WHERE name = 'welfare'")
	if result then
		...
		if (factionID) and (factionID > 0) then
			...
			if (theTeam) then
				...
				for k,v in ipairs(getPlayersInTeam(theTeam)) do
					outputChatBox("==> Tüm birlik araçları " .. (hiddenAdmin == 0 and adminTitle .. " " .. username or "Gizli Yetkili") .. " tarafından yenilendi. <==", v)
				end
			...
			outputChatBox("KULLANIM: /" .. commandName .. " [Birlik ID]", thePlayer, 255, 194, 14, false)
		end
	end
end
