...
------------------------------------------
function display_person (charactername, age, char_weight, char_height, gender, licenses, pdjail, dob, ethnicity, phone, occupation, address, photo, details, created_by, wanted, wanted_by, wanted_details, charid, vehicles, properties, crimes, pilotEvents, pilotDetails, pilotLicenses)
	local window = { }
	local width = 700
	local height = 600
	local x = SCREEN_X / 2 - width / 2
	local y = SCREEN_Y / 2 - height / 2
	window.window = guiCreateWindow(x, y, width, height, tostring(getElementData(localPlayer, "mdc_org")) .. " - MDC Search - Person: " ..  charactername:gsub("_", " "), false)
	...
	, false)
	if(getElementData(localPlayer, "mdc_org") == "FAA") then
		...
		addEventHandler("onClientGUIClick", window.removeButton, 
			function ()
				...
				if tonumber(crime_id) then
					if tonumber(getElementData(getLocalPlayer(), "mdc_admin")) > 1 or crime_officer == getElementData(getLocalPlayer(), "mdc_account") then
						triggerServerEvent(resourceName .. ":remove_pilot_event", getLocalPlayer(), charactername, crime_id)
					else
	...
	if(getElementData(localPlayer, "mdc_org") == "FAA") then
		...
		if canSeeVehicles["FAA"] then
			window.vehicleTab	= guiCreateTab("Owned Aircrafts", window.panel)
		end
	else
		window.panel		= guiCreateTabPanel (width - 200, 190, 190, 200, false, window.window)
		if canSeeVehicles[getElementData(localPlayer, "mdc_org")] then
			window.vehicleTab	= guiCreateTab("Vehicles", window.panel)
		end
		if canSeeProperties[getElementData(localPlayer, "mdc_org")] then
			window.propertyTab	= guiCreateTab("Properties", window.panel)
		end
	end
	
	--Vehicles
	if(getElementData(localPlayer, "mdc_org") == "FAA") and canSeeVehicles["FAA"] then
	...
	--Licenses
	if canSeeLicenses[getElementData(localPlayer, "mdc_org")] then
	...
	--PILOT STUFF
	if canSeePilotStuff[getElementData(localPlayer, "mdc_org")] then
		...
		addEventHandler("onClientGUIClick", window.pilotDetailsButton,
			function ()
				...
				destroyElement(window.window)
				window = { }
				triggerServerEvent(resourceName .. ":update_pilot_details", getLocalPlayer(), charid, charactername, pilotDetails)
				guiSetInputEnabled (false)
			end
		, false)
		...
		local myAdminLevel = getAdminLevel() or 1
		if getElementData(localPlayer, "mdc_org") == "FAA" then
				...
				addEventHandler("onClientGUIClick", window.pilotLicenseRevokeButton,
					function ()
						...
						if tonumber(license_uid) then
							local meAdmin = tonumber(getElementData(getLocalPlayer(), "mdc_admin"))
							if meAdmin == 2 or meAdmin == 3 then
								...
								triggerServerEvent(resourceName .. ":remove_pilot_license", getLocalPlayer(), charid, charactername, license_uid, licensetext)
							else
								remove_pilot_event_noperm(charactername)
							end
						else
...
function add_pilot_event(charid, charactername)
	...
	addEventHandler("onClientGUIClick", window.addButton, 
		function ()
			...
			destroyElement(window.window)
			window = { }
			triggerServerEvent(resourceName .. ":add_pilot_event", getLocalPlayer(), charid, charactername, crime, punishment)
		end
	, false)
...
function add_pilot_license(charid, charactername)
	...
	window.LicenseCombo = guiCreateComboBox (80, y, width - 90, 180, "-Select license-", false, window.window)
		...
		guiComboBoxAddItem(window.LicenseCombo, "AGC")
	...
	addEventHandler("onClientGUIClick", window.addLicenseButton, 
		function ()
			...
			destroyElement(window.window)
			window = { }
			triggerServerEvent(resourceName .. ":add_pilot_license", getLocalPlayer(), charid, charactername, license, aircraft)
		end
	, false)
