...
function login ()
	if hasMDCPermissions() then
		...
		window.window = guiCreateWindow(x, y, width, height, "Los Santos County Sheriff Monitor", false)
		...
		outputChatBox("[!]#FFFFFF Bir bilgisayara yakın değilsiniz.", 255, 155, 155, true)
	end
end
...
function createLoginWindow()
	...
	window.exitButton = guiCreateButton((width - 20) / 2 + 10, 150, (width - 20) / 2, 30, "Çıkış Yap", false, window.window)
	addEventHandler("onClientGUIClick", window.exitButton, 
		function ()
			showCursor(false, false)
			guiSetInputEnabled (false)
...
function mdc_errorWin (text)
	...
	local x = SCREEN_X / 2 - width / 2
	local y = SCREEN_Y / 2 - height / 2
	window.dialog.window = guiCreateWindow(x, y, width, height, "MDC Hatası", false)
	
	window.dialog.errorLabel = guiCreateLabel(10, 30, width - 20, 20, tostring(text), false, window.dialog.window)
	
	window.dialog.closeButton = guiCreateButton(10, 60, width - 20, 40, "Kapat", false, window.dialog.window)
...
function mdc_confirmWin (text)
	...
	local x = SCREEN_X / 2 - width / 2
	local y = SCREEN_Y / 2 - height / 2
	window.confirmWin.window = guiCreateWindow(x, y, width, height, "MDC Onay", false)
