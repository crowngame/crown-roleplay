...
function cacheOnStart()
	--mdc_users
	--[[
	local result = exports.cr_mysql:query("SELECT * FROM `mdc_users`")
	if result then
		while true do
	...
	local result = exports.cr_mysql:query("SELECT * FROM `mdc_faa_licenses`")
	if result then
		local theTable = {}
		while true do
...
function main ()
	...
	if (warrantResult) then
		...
		while true do
			...
			local account = nil
			local accountResult = exports.cr_mysql:query("SELECT `user`,`organization` FROM mdc_users WHERE `id` = '" ..  exports.cr_mysql:escape_string(row.wanted_by)  .. "'")
	...
	local callsResult = exports.cr_mysql:query("SELECT m.id, m.number, m.description, m.timestamp, c.charactername FROM `mdc_calls` m LEFT OUTER JOIN `mdc_criminals` t ON m.number = t.phone LEFT OUTER JOIN characters c ON c.id = t.character ORDER BY m.id DESC LIMIT 20")
	if (callsResult) then
		while true do
...
function search(query, queryType)
	...
	elseif queryType == 0 then --Person
		...
		if exports.cr_mysql:num_rows(result) > 0 then
			...
			local vehicles = { }
			local result4 = exports.cr_mysql:query("SELECT v.id, v.model, `plate`, c.vehbrand, c.vehmodel, c.vehyear FROM `vehicles` v LEFT JOIN vehicles_shop c ON v.vehicle_shop_id = c.id WHERE `owner` = '" ..  character.id  .. "' AND deleted = 0 AND registered = 1")
			if (result4) then
				local count = 1
			...
			if(getElementData(source, "mdc_org") == "FAA") then
				local result7 = exports.cr_mysql:query("SELECT * FROM `mdc_faa_events` WHERE `character` = '" ..  character.id  .. "' ORDER BY `id` DESC")
				if (result7) then
					local count = 1
					while true do
						row = exports.cr_mysql:fetch_assoc(result7)
						if not row then break end
						pilotEvents[count] = { }
					...
					exports.cr_mysql:free_result(result7)
				end
				...
				if mdc_faa_licenses then
					...
					local result8 = exports.cr_mysql:query("SELECT * FROM `mdc_faa_licenses` WHERE `character` = '" ..  character.id  .. "' ORDER BY `id` ASC")
					if (result8) then
						local count = 1
						while true do
							row = exports.cr_mysql:fetch_assoc(result8)
							if not row then break end
							pilotLicenses[count] = { }
							pilotLicenses[count][1] = row.id
							pilotLicenses[count][2] = row.license
						...
						exports.cr_mysql:free_result(result8)
					end
				end
			end
	...
	elseif queryType == 1 or queryType == 3 then --Vehicle
		local q = ""
		if queryType == 1 then
			q = "v.plate = '" ..  exports.cr_mysql:escape_string(query) .. "'"
		elseif queryType == 3 then
			q = "v.id = '" ..  exports.cr_mysql:escape_string(query) .. "'"
		else
			return false
		end
		...
		local result = exports.cr_mysql:query("SELECT v.*, c.vehbrand, c.vehmodel, c.vehyear FROM `vehicles` v LEFT JOIN vehicles_shop c ON v.vehicle_shop_id = c.id WHERE " .. q .. " AND deleted = 0 AND registered = 1") --Fetch the information from the database 
		if exports.cr_mysql:num_rows(result) > 0 then
...
function add_crime(charid, charactername, crime, punishment)
	...
	local addCrime = exports.cr_mysql:query_insert_free("INSERT INTO `mdc_crimes` (`crime`, `punishment`, `character`, `officer`, `timestamp`) VALUES ('" .. exports.cr_mysql:escape_string(crime) .. "','" .. exports.cr_mysql:escape_string(punishment) .. "','" .. charid .. "','" .. officer .. "', '" .. timestamp .. "')")
...
function add_apb(description, person)
	...
	local query = exports.cr_mysql:query_insert_free("INSERT INTO `mdc_apb` (`person_involved`, `description`, `doneby`, `time`, `organization`) VALUES ('" .. exports.cr_mysql:escape_string(person) .. "','" .. exports.cr_mysql:escape_string(description) .. "','" .. officer .. "', '" .. timestamp .. "', '" .. exports.cr_mysql:escape_string(org) .. "')")
	if query then
		main()
	end
end
...
function add_pilot_event(charid, charactername, crime, punishment)
	...
	local time = getRealTime()
	local timestamp = time.timestamp
	local query = exports.cr_mysql:query_insert_free("INSERT INTO `mdc_faa_events` (`crime`, `punishment`, `character`, `officer`, `timestamp`) VALUES ('" .. exports.cr_mysql:escape_string(crime) .. "','" .. exports.cr_mysql:escape_string(punishment) .. "','" .. charid .. "','" .. officer .. "', '" .. timestamp .. "')")
	if query then
		search(charactername, 0)
	end
end
...
function add_pilot_license(charid, charactername, license, aircraft)
	...
	local query = exports.cr_mysql:query_insert_free("INSERT INTO `mdc_faa_licenses` (`license`, `value`, `character`, `officer`, `timestamp`) VALUES ('" .. exports.cr_mysql:escape_string(license) .. "'," .. exports.cr_mysql:escape_string(tostring(aircraft)) .. ",'" .. charid .. "','" .. officer .. "', '" .. timestamp .. "')")
	if query then
		if mdc_faa_licenses then
...
function remove_pilot_event(charactername, crime_id)
	local query = exports.cr_mysql:query("DELETE FROM `mdc_faa_events` WHERE `id` = '" .. crime_id .. "'")
	if query then
		search(charactername, 0)
	end
end
...
function remove_pilot_license(charid, charactername, license_uid, licensetext)
	local query = exports.cr_mysql:query("DELETE FROM `mdc_faa_licenses` WHERE `id` = '" .. exports.cr_mysql:escape_string(tostring(license_uid)) .. "'")
	if query then
		local match
		if mdc_faa_licenses then
			for k,v in ipairs(mdc_faa_licenses) do
...
function update_pilot_details(charid, charactername, pilotDetails)
	pilotDetails = exports.cr_mysql:escape_string(pilotDetails)	
	local qUpdate = exports.cr_mysql:query("UPDATE `mdc_criminals` SET `pilot_details` = '" .. pilotDetails .. "' WHERE `character` = '" .. exports.cr_mysql:escape_string(charid) .. "' ")
	if qUpdate then
		search(charactername, 0)
	end
end
...
function system_admin()
	...
	local query = exports.cr_mysql:query("SELECT * FROM `mdc_users` WHERE `organization`='" .. exports.cr_mysql:escape_string(org) .. "' ORDER BY id ASC")
	if (query) then
		local rows = { }
		local count = 1
...
function create_account(user, pass, level, organization)
	...
	local query = "INSERT INTO `mdc_users` (`user`, `pass`, `level`, `organization`) VALUES ('" .. exports.cr_mysql:escape_string(user) .. "','" .. exports.cr_mysql:escape_string(pass) .. "','" .. exports.cr_mysql:escape_string(level + 1) .. "','" .. exports.cr_mysql:escape_string(organization) .. "')"
	...
	--else
		...
		if charID then
			if mdc_faa_licenses then
				...
				local result8 = exports.cr_mysql:query("SELECT `license`, `value` FROM `mdc_faa_licenses` WHERE `character` = " ..  charID  .. " ORDER BY `id` ASC")
				if (result8) then
					local count = 1
					while true do
						row = exports.cr_mysql:fetch_assoc(result8)
...
function refreshPilotLicensesCache(thePlayer, commandName)
	if (getElementData(thePlayer, "faction") == 47 and getElementData(thePlayer, "factionleader") == 1) or exports.cr_integration:isPlayerTrialAdmin(thePlayer) or exports.cr_integration:isPlayerScripter(thePlayer) then
...
function updateVehicleStolen(vehicleID)
	exports.cr_mysql:query_free('UPDATE `vehicles` SET `stolen` = 1 - `stolen` WHERE `id` = ' .. vehicleID)
end
