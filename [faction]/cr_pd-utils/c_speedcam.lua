function createSpeedcam ()
    if getElementData(localPlayer, "speedradar_chat") == false and not speedcamMeter then
        speedFont = guiCreateFont("speedcam/digital-7.ttf", 38)

        speedcamMeter = guiCreateStaticImage(0.75, 0.75, 0.24, 0.17, "speedcam/gps.png", true)
        targetSpeed = guiCreateLabel(0.15, 0.36, 0.21, 0.25, "0", true, speedcamMeter)
        setSpeed = guiCreateLabel(0.40, 0.36, 0.21, 0.25, "", true, speedcamMeter)
        allowedSpeed = guiCreateLabel(0.66, 0.36, 0.21, 0.25, "", true, speedcamMeter)
...
function getAllData ()
    ...
    local ownSpeedValue = exports.in_global:getVehicleVelocity(vehicle, getLocalPlayer())
