...
function(player, cmd, speed)
    ...
    if vehicle then
        if (fID == 1) or (fID == 50) then
            if exports.cr_global:hasItem(vehicle, 284) then
                if getElementData(vehicle, "anpr:state") then
                    ...
                    outputChatBox("[!]#FFFFFF ANPR Sistemi bu kruvazörde devre dışı bırakıldı.", player, 255, 180, 20, true)
                    ...
                    if not isANPRExisting(player) then
                        ...
                        outputChatBox("[!]#FFFFFF ANPR Sistemini açtınız.", player, 255, 180, 20, true)
                        --triggerClientEvent(player, "anprON", player)
                        playSoundFrontEnd(player, 45)
                    else
                        outputChatBox("KULLANIM: /" .. cmd, player, 255, 180, 20, false)
                    end    
                end 
            else
                outputChatBox("[!]#FFFFFF ANPR sistemine sahip bir kanun uygulama aracında olmalısınız.", player, 255, 0, 0, true)    
            end    
        else
                        ...
                        if isVerifiedElement(radar, "vehicle") and getElementData(radar, "anpr:state") then
                            ...
                            if hasVehicleANPR(hitElement) then
                                for seat,player in pairs(getVehicleOccupants(radar)) do
                                    ...
                                    setElementData(player, "anpr:vehicleName", exports.cr_global:getVehicleName(hitElement), true)
                                    setElementData(player, "anpr:vehicleName", exports.cr_global:getVehicleName(hitElement), true)
                                    setElementData(player, "anpr:vehicleDirection", direction, true)
                                    outputChatBox("[RADAR] A ANPR hit was recorded on a " .. getColorName(c1, c2) .. " " .. exports.cr_global:getVehicleName(hitElement) .. " with plates " .. getElementData(hitElement, "plate") .. ".", player, 255, 0, 0, false)
...
function hasVehicleANPR(theVehicle)
	if getElementType(theVehicle) == "vehicle" then
		local anpr = exports.cr_mdc:getANPRTable()
