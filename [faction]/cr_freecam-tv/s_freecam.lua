...
function tv(player)
	local hasTV = exports.cr_global:hasItem(player, 104, 1)
	...
	function(player)
		if getElementData(player, "faction") == 20 then
			if not isTVRunning() then
				...
				exports.cr_logs:dbLog(player, 23, player, "TV START")
				watching = 0
				earnings = 0
		...
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				...
				for k, v in ipairs(getElementsByType("player")) do
					...
					if getElementData(v, "faction") == 20 then
						outputChatBox("[TV] Max. Viewers: " .. watching .. ", Earnings: $" .. exports.cr_global:formatMoney(earnings), v, 200, 100, 200)
					end
				end
				
				exports.cr_logs:dbLog(player, 23, player, "TV STOP - VIEWERS: " .. watching .. " EARNINGS: " .. earnings)
			else
	...
	function(player)
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				...
				outputChatBox("[TV] Max. Viewers: " .. watching .. ", Earnings: $" .. exports.cr_global:formatMoney(earnings), player, 200, 100, 200)
			else
				outputChatBox("There's no TV Show running.", player, 255, 0, 0)
			end
		end
...
function add(affectedPlayers)
	...
	if isTVRunning() and shownto > 0 then
		...
		exports.cr_global:giveMoney(getTeamFromName"San TV", LSN_MONEY * shownto)
		--exports.cr_logs:dbLog(player, 23, player, "TV $" .. (10 * shownto) .. " " .. message)
	else
		--exports.cr_logs:dbLog(player, 23, player, "TV OFF " .. message)
	end
end